...
struct _mjData
{
    ...
    mjtNum solverstat[4];           // solvers statistics... replace with struct ???
    ...
    int*      efc_signature;        // unique signature for warmstart           (njmax x 1)
...
// generic callback
typedef void (*mjfCallback) (const mjModel* m, mjData* d);
