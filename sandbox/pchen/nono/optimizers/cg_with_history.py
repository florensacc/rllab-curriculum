...
class ConjugateGradientOptimizerWithHistory(Serializable):
    """
    ...
    def __init__(
        ...
        self._backtrack_on_history = backtrack_on_history
    ...
    def optimize(self, inputs, extra_inputs=None, subsample_grouped_inputs=None):
        ...
        # after calculating the descent step with current batch of experience
        #  we consult the history book to make sure we don't overstep ourselves
        bak_inputs = inputs
        if self._history is None:
            self._history = inputs
        else:
            self._history = [
                np.concatenate([inp, his], axis=0)[:self._history_size]
                for inp, his in zip(inputs, self._history)
            ]
            from sandbox.pchen.nono.ltrpo import policy
            inputs = (self._history)
            _, act_info_dict = policy.get_actions(inputs[0])
            inputs[3] = act_info_dict["mean"]
        ...
        if not self._backtrack_on_history:
            inputs = bak_inputs
