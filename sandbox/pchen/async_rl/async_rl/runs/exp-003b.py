"""
Frostbite w/ A3C without entropy bonus
Repeat exp-003, with fewer processes and longer gaps between evaluation
LastSyncTimeGap is underestimated here. As even when not syncing, the gap was recorded.
"""
...
vg = VariantGenerator()
vg.add("game",["frostbite"])
vg.add("entropy_bonus",[0,0.001])
...
for v in vg.variants():
    ...
    locked_stats = True
    sync_t_gap_limit = 1000 # related to n_processes
    ...
    if bonus_count_target == "ram":
        ...
        preprocessor = ImageVectorizePreprocessor(
            ...
            width=img_width,
            height=img_height,
            use_current_image=use_current_image,
        )
        count_target_dim = preprocessor.output_dim
