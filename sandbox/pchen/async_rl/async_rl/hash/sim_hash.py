...
from sandbox.pchen.async_rl.async_rl.hash.base import Hash
...
class SimHash(Hash,Shareable,Picklable):
    def __init__(self,item_dim, dim_key=128, bucket_sizes=None):
        ...
        self.unpicklable_list = ["tables","shared_params"]
    ...
    def extract_shared_params(self):
        return {
            "tables": mp.RawArray('l',self.tables.ravel().astype(int)),
        }
    ...
    def set_shared_params(self,params):
        ...
        ).reshape(self.tables.shape)

    def process_copy(self):
        new = SimHash(**self.init_params)
        new.projection_matrix = np.copy(self.projection_matrix)
    ...
    def __getstate__(self):
        ...
        # make a copy of the hash table in shared memory
        state["tables"] = np.copy(self.tables)
        return state
