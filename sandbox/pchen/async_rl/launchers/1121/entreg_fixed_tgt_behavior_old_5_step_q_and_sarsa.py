...
class VG(VariantGenerator):
    ...
    def game(self, ):
        ...
        return ["beam_rider", "qbert", "seaquest"]
    ...
    @variant
    def temp_init(self, adaptive_entropy):
        if adaptive_entropy:
            return [1e-2, ]
        else:
            return [1e-2, 1e-1, 1e0]
...
for v in variants[:]:
    ...
    run_experiment_lite(
        algo.train(),
        exp_prefix="1121_entreg_fixbehave_old5_q_and_sarsa",# use the batch after 1am
        seed=v["seed"],
        variant=v,
        # mode="local",
        mode="ec2",
