...
for i in range(args.nr_gpu):
    with tf.device('/gpu:%d' % i):
        x_noisy = xs[i] + tf.random_uniform(shape=(args.batch_size, )+obs_shape, minval=-1./255., maxval=1./255., dtype=tf.float32)

        # train
        gen_par = model(x_noisy, hs[i], ema=None, dropout_p=args.dropout_p, **model_opt)
        loss_gen.append(nn.continuous_mix_logistic_loss(x_noisy, gen_par))
        # gradients
        grads.append(tf.gradients(loss_gen[i], all_params))
        # test
        gen_par = model(x_noisy, hs[i], ema=ema, dropout_p=0., **model_opt)
        loss_gen_test.append(nn.continuous_mix_logistic_loss(x_noisy, gen_par))
