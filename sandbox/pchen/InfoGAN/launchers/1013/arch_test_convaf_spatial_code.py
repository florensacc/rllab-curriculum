...
# better than no slowkl version in terms of kl and bits/dim
# no difference between 2-step & 4-step version, which means p(z) is not the limitation
#   of current capacity

# THINK: curves look spiky, maybe reduce lr?
# THINK: what is limiting the capacity? try different axes
...
# trying different filter size in pixelcnn
# experiment with efficient gpu usage
...
class VG(VariantGenerator):
    ...
    @variant(hide=False)
    def ar_filter_size(self):
        return [5,]

    @variant(hide=False)
    def ar_depth(self, ar_filter_size):
        # if ar_filter_size == 4:
        #     yield 8
        # elif ar_filter_size == 5:
        #     yield 5
        # else:
    ...
    # @variant
    # def arch(self):
    #     return [True, False]#[12, 32]
    #
    # @variant
    # def arch_nr(self, arch):
    #     if arch:
    #         return [2, 6]#[12, 32]
    #     return [1]
    #
    # @variant
    # def arch_depth(self, arch_nr):
    #     if arch_nr == 2:
    #         return [2, 4]#[12, 32]
    #     elif arch_nr == 6:
    #         return [2, 4]
    ...
    # @variant
    # def arch(self):
    #     return ["gated", "op", "mul_gated"]#[12, 32]
    #
    #
    # @variant
    # def arch_nr(self, arch):
...
for v in variants[i:i+1]:
    ...
    # with skip_if_exception():
        ...
        tgt_ar_dist = DistAR(
            3,
            tgt_dist,
            depth=v["arch_depth"],#2,
            neuron_ratio=v["arch_nr"],
            linear_context=True,
            # linear_context="op" not in v["arch"],
            # gating_context="gat" in v["arch"],
            # op_context="op" in v["arch"],
            # mul_gating="mul" in v["arch"],
        )
        ar_conv_dist = ConvAR(
            # tgt_dist=Bernoulli(1),
            # tgt_dist=tgt_dist,
            tgt_dist=tgt_ar_dist if v["arch"] else tgt_dist,
            shape=(32, 32, 3),
        ...
        run_experiment_lite(
            algo.train(),
            exp_prefix="1013_ar_channels_test",
            seed=v["seed"],
            variant=v,
            mode="local",
            # mode="lab_kube",
