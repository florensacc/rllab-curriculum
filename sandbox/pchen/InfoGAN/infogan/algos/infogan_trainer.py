...
class InfoGANTrainer(object):
    def __init__(self,
        ...
        if cont_info_reg_coeff is None:
            self.cont_info_reg_coeff = info_reg_coeff
        else:
    ...
    def init_opt(self):
        ...
        with pt.defaults_scope(phase=pt.Phase.train):
            ...
            for idx, dist_info in enumerate(self.model.reg_latent_dist.split_dist_info(fake_reg_z_dist_info)):
                # tf.scalar_summary("max_std_%d" % idx, tf.reduce_max(dist_info["stddev"]))
                # tf.scalar_summary("min_std_%d" % idx, tf.reduce_min(dist_info["stddev"]))
            ...
            # tf.scalar_summary("MI", mi_est)
            self.log_vars.append(("MI", mi_est))
            self.log_vars.append(("CrossEnt", cross_ent))

            # discriminator_loss -= self.info_reg_coeff * disc_mi_est + self.cont_info_reg_coeff * cont_mi_est
            # generator_loss -= self.info_reg_coeff * disc_mi_est + self.cont_info_reg_coeff * cont_mi_est
