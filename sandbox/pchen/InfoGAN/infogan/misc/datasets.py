...
import os, shutil, re, string, urllib.request, urllib.parse, urllib.error, fnmatch
...
def _download_mnist_realval(dataset):
    ...
    origin = (
        ...
        'https://s3-us-west-1.amazonaws.com/pchen/datasets/mnist.pkl.gz'
    )
    print('Downloading data from %s' % origin)
...
def _download_mnist_binarized(datapath):
    ...
    url = "https://s3-us-west-1.amazonaws.com/pchen/datasets/binarized_mnist.gz"
    fn = datapath + '/mnist.pkl.gz'
...
def load_rotten_tomatoes(dataset=_get_datafolder_path()+'/rotten_tomatoes/',
    ...
    if isinstance(vocab_size, int):
        ...
        char_counts_sorted = sorted(
            list(char_counts.items()), key=lambda x:x[1], reverse=True)

        chars, counts = list(zip(*char_counts_sorted[:vocab_size]))
...
def _download_and_extract_stl10(dest_directory):
    ...
    if not os.path.exists(filepath):
        def _progress(count, block_size, total_size):
            ...
            sys.stdout.flush()
        filepath, _ = urllib.request.urlretrieve(origin, filepath, reporthook=_progress)
...
def load_stl10(
    ...
    def read_all_images(path_to_data):
        print("Loading %s" % path_to_data, end=' ')
    ...
    def read_labels(path_to_labels):
        print("Loading %s" % path_to_labels, end=' ')
