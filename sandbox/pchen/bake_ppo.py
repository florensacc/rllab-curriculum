...
class BakePPO(BatchPolopt):
    ...
    def __init__(
        ...
        super(BakePPO, self).__init__(**kwargs)
    ...
    def init_opt(self, mdp, policy, baseline):
        ...
        pred_var = policy.get_pred(input_var, action_var)
        ...
        pred_loss = TT.mean(TT.square(pred_var[:-1] - input_var[1:]))
        pred_obj = pred_loss + penalty_var * mean_kl
        ...
        grads = theano.gradient.grad(
            surr_obj, policy.get_params(trainable=True), disconnected_inputs='warn')
        f_surr_kl = compile_function(
            input_list, [surr_obj, surr_loss, mean_kl])
        f_grads = compile_function(input_list, grads)

        pred_grads = theano.gradient.grad(
            pred_obj, policy.get_params(trainable=True), disconnected_inputs='warn')
        pred_f_surr_kl = compile_function(
            input_list, [pred_obj, pred_loss, mean_kl])
        pred_f_grads = compile_function(input_list, pred_grads)
        ...
        return dict(
            f_surr_kl=f_surr_kl,
            f_grads=f_grads,
            pred_f_surr_kl=pred_f_surr_kl,
            pred_f_grads=pred_f_grads,
            penalty=penalty,
        )

    def go(self,itr,policy,samples_data,opt_info,f_surr_kl,f_grads):
