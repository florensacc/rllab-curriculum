...
def run_task(v):
    ...
    if v['smart_init']:  # TODO: adapt to start generation
    ...
    labels = label_states(initial_goals, env, policy, v['horizon'], as_goals=False, n_traj=v['n_traj'], key='goal_reached')
    ...
    for outer_iter in range(1, v['outer_iters']):
        ...
        raw_starts, _ = gan.sample_states_with_noise(v['num_new_goals'])

        if v['replay_buffer'] and outer_iter > 0 and all_starts.size > 0:
            old_starts = all_starts.sample(v['num_old_goals'])
