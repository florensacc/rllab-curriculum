...
from sandbox.young_clgan.envs.init_sampler.base import update_env_init_generator
from sandbox.young_clgan.envs.init_sampler.base import FixedInitGenerator
...
def label_inits(inits, env, policy, horizon, min_reward=0.1, max_reward=0.9,
                old_rewards=None, improvement_threshold=None, n_traj=1, n_processes=-1):
    print("Evaluating inits after training")
    mean_rewards = evaluate_inits(inits, env, policy, horizon, n_traj, n_processes=n_processes)

    print("Computing inits labels")
...
def evaluate_inits(inits, env, policy, horizon, n_traj=1, n_processes=-1):
    evaluate_init_wrapper = FunctionWrapper(
        evaluate_init,
        env=env,
        policy=policy,
        horizon=horizon,
...
def evaluate_init(init, env, policy, horizon, n_traj=1):
    total_rewards = []
    paths = []
    update_env_init_generator(env, FixedInitGenerator(init))
...
def evaluate_init_env(env, policy, horizon, n_inits=10, n_traj=1, **kwargs):  # you can pass the html report in kwargs
    paths = [rollout(env=env, agent=policy, max_path_length=horizon) for _ in range(n_inits)]
