...
class InitExplorationEnv(InitEnvAngle, ProxyEnv, Serializable):
    ...
    def reset(self, reset_init=True, reset_inner=True):
        # print("\n\nresetting the InitExplorationEnv, current obs: ", self.get_current_obs())
        if reset_init:
            # print("resetting init, before self.current_init=", self.current_init)
            self.update_init()
            # print("after self.current_init=", self.current_init)
        if reset_inner:
            # print('resetting innner..')
    ...
    def step(self, action):
        # print("BASE: the action taken is: ", action)
...
def update_env_init_generator(env, goal_generator):
    """ Update the goal generator for normalized environment. """
    obj = env
    while not hasattr(obj, 'update_init_generator') and hasattr(obj, 'wrapped_env'):
        obj = obj.wrapped_env
    if hasattr(obj, 'update_init_generator'):
        return obj.update_init_generator(goal_generator)
    else:
        raise NotImplementedError('Unsupported environment')


def get_init_observation(env):
...
# TODO: make the env not terminate when reaching the goal to produce full rollouts?
def generate_initial_inits(env, policy, init_range, horizon=500, size=10000):
    current_goal = get_current_goal(env)
    goal_dim = np.array(current_goal).shape

    done = False
    obs = env.reset()
    inits = [get_init_observation(env)]
    steps = 0
    while len(inits) < size:
        ...
        if done or steps >= horizon:
            ...
            update_env_init_generator(
                env,
                FixedInitGenerator(
                    np.random.uniform(-init_range, init_range, goal_dim)
                )
            )
            obs = env.reset()
            inits.append(get_init_observation(env))
        else:
