...
class ConjugateGradientOptimizer_precond(Serializable):
    """
    ...
    def update_opt(self, loss, target, leq_constraint,
                   inputs, ll, mean_squared_grads=None,  extra_inputs=None, constraint_name="constraint", *args,
                   **kwargs):
        """
        ...
        # grad_ll = theano.gradient.jacobian(ll, wrt=params)
        grad_ll = theano.grad(TT.mean(ll), wrt=params)
        flat_grad_ll = ext.flatten_tensor_variables(grad_ll)
        mean_squared_grads = TT.mean(flat_grad_ll)

        Minvx = xs * mean_squared_grads**(-1)
        self._target = target
        ...
        self._opt_fun = ext.lazydict(
            ...
            f_Minvx=lambda: ext.compile_function(
                inputs=inputs + extra_inputs + xs,
                outputs=Minvx,
                log_name="mean_squared_grads",
                mode=mode,
            ),
        )
    ...
    def optimize(self, inputs, extra_inputs=None):
        ...
        def Minvx(x):
            xs = tuple(self._target.flat_to_params(x, trainable=True))
            #     rop = f_Hx_rop(*(inputs + xs))
            plain = self._opt_fun["f_Minvx"](*(subsample_inputs + extra_inputs + xs)) + self._reg_coeff * x
            # assert np.allclose(rop, plain)
            return plain
            # alternatively we can do finite difference on flat_grad
        ...
        descent_direction = krylov.preconditioned_cg(Hx, Minvx, flat_g, cg_iters=self._cg_iters)
