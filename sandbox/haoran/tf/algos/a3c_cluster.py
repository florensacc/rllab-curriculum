...
class A3C(RLAlgorithm, Serializable):
    ...
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            worker_stats_queue = mp.Queue()
            for worker_id in range(self.n_workers):
                self.worker_envs.append(pickle.loads(pickle.dumps(self.env)))
                p = mp.Process(target=run_algo, args=(self, self.env, shared_T, worker_stats_queue))
                p.start()
                workers.append(p)
            ...
            try:
                while True:
                    time.sleep(0.01)
                    if shared_T.value - last_T >= self.epoch_length or shared_T.value >= self.max_T:
                        all_stats = []
                        ...
                        # log new results
                        last_T = shared_T.value
                        epoch = last_T / self.epoch_length
                        logger.record_tabular("Epoch", epoch)
                        logger.record_tabular("T", last_T)
                        kvs = dict()
