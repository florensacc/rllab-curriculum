...
Continue exp-026f, with larger batch size; include freeway
"""
# imports -----------------------------------------------------
""" baseline """
...
class VG(VariantGenerator):
    ...
    @variant
    def game(self):
        return ["frostbite", "freeway", "venture","montezuma_revenge","solaris","gravitar"]
...
for v in variants:
    ...
    env = AtariEnv(
        ...
        recorded_rgb_image_scale=scale,
    )
    ...
    bass = BassFeatureExtractor(
        image_shape=(
            int(np.round(210*scale)),
            int(np.round(160*scale)),
            3
        ),
        cell_size=cell_size,
        n_bin=n_bin,
