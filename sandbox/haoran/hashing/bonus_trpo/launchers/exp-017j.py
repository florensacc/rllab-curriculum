"""
Continue exp-017j
- try HOGHash, with parameter fine-tuned by human play
"""
# imports -----------------------------------------------------
""" baseline """
...
from sandbox.haoran.hashing.bonus_trpo.bonus_evaluators.preprocessor.hog_rgb_preprocessor import HOGRGBPreprocessor
from sandbox.haoran.hashing.bonus_trpo.bonus_evaluators.preprocessor.slicing_preprocessor import SlicingPreprocessor
from sandbox.haoran.hashing.bonus_trpo.bonus_evaluators.hash.hog_hash import HOGHash
...
for v in variants:
    ...
    count_rgb = False
    if count_rgb:
        count_target="rgb_images" # make sure memory is enough
    else:
        count_target="observations"
    num_orientations=v["num_orientations"]
    cell_size=v["cell_size"]
    threshold=1./num_orientations * v["threshold_scale"]
    transpose=not count_rgb
    oriented=True
    ...
    if count_rgb:
        ...
        state_preprocessor = HOGRGBPreprocessor(
            img_height=210,
            img_width=160,
            img_channel=3,
        )
