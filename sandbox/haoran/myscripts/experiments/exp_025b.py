# exp_025 is dedicated to understanding why reusing previous data does not help improve performance on Hopper
# We first find a large enough sample size for FIM, so that it doesn't become a factor of consideration.
# Also we switch to a shorter horizon to accelerate experiments.
# Here we decrease the network size from exp_025a, so that estimating FIM should be much easier.
...
# varying params
env_names = ["hopper"]
batch_size_list = np.array([10, 20]) * 1000
reuse_history_list = [
    dict(
        max_len=0,
        reweigh_type="none",
...
for env_name,batch_size,reuse_history,gae_lambda,sf in itertools.product(env_names,batch_size_list,reuse_history_list,gae_lambda_list, sf_list):
        ...
        reuse = "h{max_len}{g}{kl}".format(
            max_len=reuse_history["max_len"],
            g="g" if reuse_history["reuse_for_gradient"] else "",
            kl="kl" if reuse_history["reuse_for_kl"] else "",
        )
        exp_name_prefix="alex_{time}_{env_name}_{reuse}_sf{sf}_l{gae_lambda}_bs{batch_size}k".format(
            time=timestamp,
            env_name=env_name,
            reuse=reuse,
            sf="%.1f"%(sf),
