...
env_names = ["walker","hopper","halfcheetah","double_pendulum","ant"]
cg_iters_list = [10]
momentums = [
    dict(type="none"),
]
gae_lambdas = [0.95,0.96,0.97,0.98,0.99,1]
...
for batch_size,env_name,cg_iters,momentum,gae_lambda in \
    itertools.product(batch_sizes,env_names,cg_iters_list,momentums,gae_lambdas):
        ...
        exp_name_prefix="alex_{time}_{env_name}_lam_{gae_lambda}_bs{batch_size}k".format(
            time=timestamp,
            env_name=env_name,
            gae_lambda=gae_lambda,
