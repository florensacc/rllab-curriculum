# Repeat exp-030f,g, with very large batch size (20k) for linear baseline fitting.
...
# coupled params
n_itr_list = [10000,1000] # not that bs = 2k uses more iterations
batch_size_list = np.array([20,20]) * 1000
step_size_list = [0.001, 0.01]
input_sf_list = [0.05, 0.5]
sf_list = [1.0, 0.1] # note that sf will be multiplied by input_sf
...
for env_name,gae_lambda,reuse_history,reg_coeff, cg_iters in itertools.product(env_names,gae_lambda_list,reuse_history_list, reg_coeff_list, cg_iters_list):
    for batch_size, n_itr, step_size, input_sf, sf in zip(batch_size_list, n_itr_list, step_size_list, input_sf_list, sf_list):
