# Test the performance of VPG on small batch sizes (compared to exp-030h,i, etc.)
# zero baseline, no center_adv
# Also fixed batch_size, max_path_length, etc., as they were not specified in exp-033.
...
# ------------------------------------------------------------------------------
exp_names = []
for env_name,lr,gae_lambda in itertools.product(env_names,lr_list,gae_lambda_list):
    for batch_size, n_itr in zip(batch_size_list, n_itr_list):
        ...
        exp_name_prefix="alex_{env_name}_lr{lr}_l{gae_lambda}_bs{batch_size}k".format(
            env_name=env_name,
            lr=lr,
            gae_lambda=gae_lambda,
