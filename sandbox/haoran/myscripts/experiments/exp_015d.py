# vary both the step_size and batch_size 
# in order to understand how "online" updates can behave
# try different momentum weights
...
env_names = ["walker","hopper","halfcheetah","ant"]
step_size_list = [0.01,0.005,0.0025,0.001]
batch_size_list = np.array([20,10,5,2]) * 1000
n_itr_list = [2000,4000,8000,20000]
...
for env_name,momentum in itertools.product(env_names,momentum_list):
    for batch_size,step_size,n_itr in \
        ...
        exp_name_prefix="alex_{time}_{env_name}_m{momentum}_sf_{subsample_factor}_kl{step_size}_bs{batch_size}k".format(
            time=timestamp,
            env_name=env_name,
            momentum=momentum,
            subsample_factor=subsample_factor,
