# Continue exp-032. Reuse for kl but not for gradient
# Note that there is no point to change prev_gae_lambda, as it only contributes to gradient.
...
# varying params
env_names = ["hopper", "walker", "halfcheetah"]
batch_size_list = np.array([1]) * 1000
gae_lambda_list = [0.7,0.8,0.9]
reuse_history_list = []
for reweigh_type,prev_gae_lambda,reuse_for_kl \
    in itertools.product(["none"],[1.0],[True,False]):
...
for env_name,gae_lambda,batch_size,reuse_history in itertools.product(env_names,gae_lambda_list,batch_size_list,reuse_history_list):
