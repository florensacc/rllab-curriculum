# Use a guiding policy to provide optimal value as the baseline.
# May consider a very small gae_lambda so that the baseline has more impact.
# Beware that I changed some setting, notably max_path_length
# No stubbing
# No plotting
...
baseline_file = "data/s3/exp-024/alex_20160704_155516_hopper_h0_l1.0_bs20k_s1/baseline.pkl"

# varying params
env_names = ["hopper"]
batch_size_list = np.array([10]) * 1000
gae_lambda_list = [1.0]
...
for env_name,gae_lambda,batch_size in itertools.product(env_names,gae_lambda_list,batch_size_list):
        ...
        exp_name_prefix="alex_{time}_{env_name}_maxbaseline_l{gae_lambda}_bs{batch_size}k".format(
            time=timestamp,
            env_name=env_name,
            gae_lambda=gae_lambda,
