# Study the joint effect of regularization of FIM, batch size, and CGI, and momentum.
# environments: Hopper, Halfcheetah, Walker
# fixed regularization factor 0.1
# PS: sf is too large here; but to be consistent with exp_020, we still use 1.0
# Here we fix momentum but vary regularization.
...
# varying params
env_names = ["hopper", "halfcheetah", "walker"]
reg_coeff_list = [0.01, 0.001, 1e-4, 1e-5]
cg_iters_list = [10,100,500]
batch_size_list = np.array([20,40]) * 1000
momentum_list = [0]
...
for env_name,cg_iters,batch_size,momentum,reg_coeff in itertools.product(env_names,cg_iters_list,batch_size_list,momentum_list,reg_coeff_list):
        ...
        exp_name_prefix="alex_{time}_{env_name}_cgi{cg_iters}_reg{reg_coeff}_m{momentum}_bs{batch_size}k".format(
            time=timestamp,
            env_name=env_name,
            cg_iters=cg_iters,
            reg_coeff="%.0e"%(reg_coeff),
            momentum="%.1f"%(momentum),
