...
+ Have three walls blocking motion. Allow only forward motion.
"""
# imports -----------------------------------------------------
...
puddles = [
    Puddle(x=-1, y=1, width=30, height=1, angle=0, cost=0,
        plot_args=dict(color=(1., 0., 0., 1.0)), hard=True),
    Puddle(x=-1, y=-2, width=30, height=1, angle=0, cost=0,
        plot_args=dict(color=(1., 0., 0., 1.0)), hard=True),
    Puddle(x=-2, y=-2, width=1, height=4, angle=0, cost=0,
        plot_args=dict(color=(1., 0., 0., 1.0)), hard=True),
]
env = TfEnv(normalize(
    AntPuddleEnv(
