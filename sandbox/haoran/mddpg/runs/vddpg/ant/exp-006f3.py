...
reward = 10 / (0.1 + dist_to_goal) + speed
This time the wall is rather big
"""
# imports -----------------------------------------------------
...
for v in variants:
    ...
    # construct objects ----------------------------------
    puddles = [
        # Puddle(x=2, y=-0.5, width=7, height=1, angle=0., cost=0,
        #     plot_args=dict(color=(1., 0., 0., 1.0)), hard=True),
        Puddle(x=1, y=-2, width=1, height=4, angle=0., cost=0,
            plot_args=dict(color=(1., 0., 0., 1.0)), hard=True),
    ]
    env_kwargs = {
