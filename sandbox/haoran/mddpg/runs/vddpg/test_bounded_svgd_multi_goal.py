...
flags.DEFINE_integer('n_particles', 50, 'Number of particles.')
flags.DEFINE_string('alg', 'vddpg', 'Algorithm.')
flags.DEFINE_string('policy', 'stochastic',
                    'Policy (DETERMINISTIC/stochastic')
flags.DEFINE_string('save_path',
    'sandbox/haoran/mddpg/runs/vddpg/test_bounded_svgd_multi_goal/',
...
class AlgTest(Alg):
    ...
    def evaluate(self, epoch, es_path_returns):
        ...
        pp = PdfPages(FLAGS.save_path + 'test_iter_' + str(self.eval_counter) + '.pdf')
...
def test():
    ...
    alg_kwargs = dict(
        ...
        n_epochs=300,  # number of SVGD steps
    ...
    with tf.Session() as sess:
        ...
        snapshot = joblib.load(
            os.path.join(
                "data/s3/mddpg/vddpg/exp-000",
                "exp-000_20170116_222754_336344_multi_goal/itr_99.pkl",
            )
        )
        qf_params = snapshot["algo"].qf.get_param_values()
        algorithm.qf.set_param_values(qf_params)
