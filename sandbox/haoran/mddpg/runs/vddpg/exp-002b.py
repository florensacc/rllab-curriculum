...
Test on multi-goal reacher
Note: should not normalize the env. Otherwise normalized_env will clip the
    actions before feeding it to the wrapped env, but then the contrl cost
    is smaller.
Repeat exp-002, with a normal terminal condition (exp-002 had a bug and
    the env never terminated)
"""
# imports -----------------------------------------------------
...
class VG(VariantGenerator):
    ...
    @variant
    def svgd_type(self):
        return ["pre-action", "action", "scaled-tanh"]
