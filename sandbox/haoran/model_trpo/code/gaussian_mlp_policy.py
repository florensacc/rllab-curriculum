...
class GaussianMLPPolicy(StochasticPolicy, LasagnePowered, Serializable):
    def __init__(
        ...
        if bound_output:
            ...
            aa = (uu-ll)/2
            bb = (uu+ll)/2
            def bounding_output_nonlinearity(x):
                return TT.tanh(x) * aa + bb
            output_nonlinearity = bounding_output_nonlinearity
        ...
        # derivatives
        mean_var_flat = mean_var.flatten()
        theta = self.get_params(trainable=True)
        flat_grads = []
        for i in range(action_dim):
            # grad w.r.t. NN params are grouped by layers; so need to flatten
            grad = theano.grad(mean_var_flat[i], wrt=theta, disconnected_inputs='warn')
            flat_grad = ext.flatten_tensor_variables(grad)
            flat_grads.append(flat_grad)
        mu_theta_var = TT.stack(flat_grads)
        self._mu_theta = ext.compile_function(
            inputs=[obs_var],
            outputs=mu_theta_var,
        )

        flat_grads = []
        for i in range(action_dim):
            # beware of the final ravel(); btw no flattenning needed
            grad = theano.grad(mean_var_flat[i], wrt=obs_var, disconnected_inputs='warn').ravel()
            flat_grads.append(grad)
        mu_s_var = TT.stack(flat_grads)
        self._mu_s = ext.compile_function(
            inputs=[obs_var],
            outputs=mu_s_var,
        )

    def get_theta(self):
        return self.get_param_values(trainable=True)

    def set_theta(self,value):
        self.set_param_values(value,trainable=True)

    def pi_theta(self,state):
        return self._mu_theta([state])

    def pi_s(self,state):
        return self._mu_s([state])
