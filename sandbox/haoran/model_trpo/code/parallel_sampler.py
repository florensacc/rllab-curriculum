...
def _worker_forward_one_step(G, init_state, init_action):
    """
    Beware: do not reset the environment to the original state.
    """
    G.env.reset(init_state)
    next_observation, reward, done, env_info = G.env.step(init_action)

    from rllab.envs.proxy_env import ProxyEnv
    if isinstance(G.env,ProxyEnv):
        next_state = G.env._wrapped_env._full_state
    else:
        next_state = G.env._full_state
