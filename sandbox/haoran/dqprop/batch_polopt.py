...
class ParallelBatchPolopt(RLAlgorithm):
    ...
    def __init__(
        ...
        self.tmax = tmax
        self.qf = qf
        self.qf_mc_weight = qf_mc_weight
        self.qf_update_weight = qf_update_weight
    ...
    def _init_par_objs_batchpolopt(self):
        ...
        barriers = SimpleContainer(
            dgnstc=mp.Barrier(n),
            log_stats=mp.Barrier(n),
        )
        self._par_objs = (shareds, barriers)
        self.baseline.init_par_objs(n_parallel=n)
        self.qf.init_par_objs(n_parallel=n)
        self.target_qf.init_par_objs(n_parallel=n)
    ...
    def train(self):
        logger.set_disable_prefix(True)
        self.init_opt()
    ...
    def _train(self, rank, shared_dict):
        ...
        for itr in range(self.current_itr, self.n_itr):
            with logger.prefix('itr #%d | ' % itr):
                ...
                if rank == 0:
                    logger.log("training the q function...")
                if self.path_replayer is not None:
                    ...
                    if len(replayed_paths) > 0:
                        self.train_qf(replayed_paths)

                if rank == 0:
                    logger.log("modifying samples...")
                self.modify_samples(samples_data)
    ...
    def train_qf(self, paths):
        """
        Q values can be trained with more iterations if necessary
        Remember to parallelize this part

        Q targets can include MC returns if necessary
        """
        # restructure data into (s,a,r,s') tuples
        ...
        rewards = tensor_utils.concat_tensor_list([path["rewards"][:-1] for path in paths])
        returns = tensor_utils.concat_tensor_list([path["returns"][:-1] for path in paths])
        n_steps_collected = len(returns)
        self.qf._optimizer.set_avg_fac(n_steps_collected)

        # compute Q update targets
        actions, policy_infos = self.policy.get_actions(next_observations)
        actions = np.array(actions,dtype=int)
        next_action_probs = policy_infos['prob'] # N x |A|
        next_qs = self.target_qf.compute_all_qs(next_observations) # N x |A|
        next_vs = np.sum(next_action_probs * next_qs, axis=1) # N
        bootstrap_targets = rewards + self.discount * next_vs
        mc_targets = returns
        w = self.qf_mc_weight
        targets = (1-w) * bootstrap_targets + w * mc_targets

        # batch gradient descent / SGD
        self.qf.fit(observations, actions, targets)

        # update the target
        if self.rank == 0:
            tau = self.qf_update_weight
            target_qf_params = self.target_qf.get_param_values(trainable=True)
            qf_params = self.qf.get_param_values(trainable=True)
            new_target_qf_params = (
                tau * target_qf_params
                + (1-tau) *  qf_params
            )
        ...
        if self.n_parallel > 1:
            self.target_qf._optimizer.synchronize_param()
    ...
    def log_diagnostics(self, itr, samples_data, dgnstc_data):
            ...
            if self.rank == 0:
                ...
                # log other stats
                # values: a dict with rank as the key
                # print(self.shared_dict)
                # Identify all stats and log them
                # for name, value_dict in self.shared_dict["stats"].items():
                ...
                #     ])
                #     print(values)
                #     logger.record_tabular_misc_stat(name, values)

                if self.path_replayer is not None:
                    logger.record_tabular(
                        'ReplayPoolSize',
                        self.path_replayer.pool_size
                    )
    ...
    def init_rank(self, rank):
        ...
        self.qf.init_rank(rank)
        self.target_qf.init_rank(rank)
    ...
    def log_stats(self, name, values):
        name_full = "stats:%s:%d"%(name, self.rank)
        self.shared_dict[name_full] = values
