...
if __name__ == '__main__':
    ...
    exp_prefix = 'init-maze-CE'
    vg = VariantGenerator()
    vg.add('test', [False])
    vg.add('n_traj', lambda test: [3])
    vg.add('sampling_res', lambda test: [1] if test else [1])
    # algorithm params
    vg.add('seed', range(20, 60, 7))
    vg.add('n_itr', [300])
    vg.add('inner_itr', lambda test: [5] if test else [2])
    ...
    vg.add('reward_dist_threshold', [0.5])
    ...
    vg.add('init_noise_level', [0.5])
    vg.add('generator_outer_iters', [5])
    vg.add('generator_discriminator_iters', [200])
    vg.add('generator_generator_iters', [10])
    vg.add('generator_noise_size', [4])
    vg.add('num_new_goals', [200])
    vg.add('num_old_goals', [100])
    vg.add('generator_generator_layers', [[256, 256]])
    vg.add('generator_discriminator_layers', [[128, 128]])
    # generator_configs
    vg.add('CE_batch_size', [128])  # proble with repeated name!!
    vg.add('CE_generator_activation', ['relu'])
    vg.add('CE_discriminator_activation', ['relu'])
    vg.add('CE_generator_optimizer', [tf.train.AdamOptimizer])
    vg.add('CE_generator_optimizer_stepSize', [0.001])
    vg.add('CE_discriminator_optimizer', [tf.train.AdamOptimizer])
    vg.add('CE_discriminator_optimizer_stepSize', [0.001])
    vg.add('CE_generator_weight_initializer', [tflearn.initializations.truncated_normal])
    vg.add('CE_generator_weight_initializer_stddev', [0.05])
    vg.add('CE_discriminator_weight_initializer', [tflearn.initializations.truncated_normal])
    vg.add('CE_discriminator_weight_initializer_stddev', [0.02])
    vg.add('CE_discriminator_batch_noise_stddev', [1e-2])
    ...
    def run_task(v):
        ...
        # CrossE
        logger.log("Instantiating the CrossE...")
        generator = CrossEntropyStateGenerator(
            state_size=np.size(v['goal']),
            state_range=v['init_range'],
            state_center=v['init_center'],
            noise_std=v['init_noise_level'],
        )
        logger.log("pretraining the generator...")
        if v['smart_init']:
            generator.pretrain(generate_initial_inits(env, policy, max_path_length=v['max_path_length']))
        else:
            generator.pretrain_uniform()

        # log first samples form the CE
        initial_inits = generator.sample_states_with_noise(v['num_new_goals'])
        ...
        for outer_itr in range(1, v['outer_itr']):
            ...
            # Sample CE
            logger.log("Sampling inits from the CE")
            raw_inits = generator.sample_states_with_noise(v['num_new_goals'])
