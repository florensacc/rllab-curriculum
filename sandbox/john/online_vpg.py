            ...
            for t in xrange(self.horizon):
                ...
                if count > 0:
                    for i_env in np.flatnonzero(new):
                        ...
                        rets[i_env] = 0
                # Filter update:
                ob_mean = (ob_mean * (count + 1) + ob.mean(axis=0, keepdims=True)) / (count + 2)
                ob_scale = np.sqrt((np.square(ob_scale) * (count + 1) + np.square(ob_mean - ob).mean(axis=0, keepdims=True) ) / (count + 2))
                count += 1
