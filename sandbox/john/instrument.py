...
def run_experiment_lite(
    ...
    if mode == "local":
        ...
        for task in batch_tasks:
            ...
            pod_dict = to_lab_kube_pod(task, docker_image=docker_image, script=script, is_gpu=use_gpu)
            ...
            if dry:
                return
            subprocess.check_call(kubecmd, shell=True)
...
def launch_ec2(params_list, exp_prefix, docker_image,
...
def to_lab_kube_pod(
    ...
    pre_commands.append("""
        while /bin/true; do
            aws s3 sync --exclude *.pkl --exclude gym_log {log_dir} {remote_log_dir} --region {aws_region}
            sleep 15
        done & echo sync initiated""".format(log_dir=log_dir, remote_log_dir=remote_log_dir,
                                             aws_region=config.AWS_REGION_NAME))
