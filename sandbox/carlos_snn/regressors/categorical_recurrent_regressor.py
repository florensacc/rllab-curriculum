...
class CategoricalRecurrentRegressor(LasagnePowered, Serializable):
    """
    ...
    def __init__(
        ...
        x_mean_var = theano.shared(
            np.zeros((1, 1,) + input_shape),  # this syntax makes the shape (1,1,*input_shape,). The first is traj
            name="x_mean",
        ...
        prob_var_all = L.get_output(l_prob, {prob_network.input_layer: normalized_xs_var})

        if predict_all:
            prob_var = prob_var_all
        else:
            # take only last dim but keep the shape
            prob_var_last = TT.reshape(prob_var_all[:, -1, :],
                                       (TT.shape(prob_var_all)[0], 1, TT.shape(prob_var_all)[2]))
            # padd along the time dimension to obtain the same shape as before
            padded_prob_var = TT.tile(prob_var_last, (1, TT.shape(prob_var_all)[1], 1))
        ...
        predicted_flat = special.to_onehot_sym(TT.flatten(TT.argmax(prob_var, axis=-1)), output_dim)
