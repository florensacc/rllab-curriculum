...
class GRULayer(L.Layer):
    """
    A gated recurrent unit implements the following update mechanism: #f_ are the NL, the * are elemt-wise mult, @ regular
    Reset gate:        r(t) = f_r(x(t) @ W_xr + h(t-1) @ W_hr + b_r)
    Update gate:       u(t) = f_u(x(t) @ W_xu + h(t-1) @ W_hu + b_u)
    Cell gate:         c(t) = f_c(x(t) @ W_xc + r(t) * (h(t-1) @ W_hc) + b_c)  #also called candidate activation
    ...
    def __init__(
            self,
            incoming,  # In Lasagne this is the previous layer OR a tuple with the expected input shape
            num_units,
            hidden_nonlinearity,
    ...
    def step(self, x, hprev):  # x is (n_batch, input_dim) and hprev is (n_batch, num_units)
    ...
    def get_output_for(self, input, **kwargs):
        ...
        shuffled_hs = hs.dimshuffle(1, 0, 2)  # set back batch as first dim: now (n_batch, n_steps?, flatten_input_dims)
        return shuffled_hs  # this is all intermediate steps also!
...
class GRUNetwork(object):
    def __init__(
            self,
            input_shape,  # this is the input shape at every step.
            output_dim,
            hidden_dim,
