...
class SoftmaxGaussianMLPPolicy(StochasticPolicy, LasagnePowered, Serializable):
    def __init__(
            self,
            env_spec,
        ...
        super(SoftmaxGaussianMLPPolicy, self).__init__(env_spec)
    ...
    def get_actions(self, observations):
        ...
        e_x = np.exp(actions - np.max(actions, axis=-1))
        softmax_actions = e_x / e_x.sum()
        latents = np.array([[]]* len(softmax_actions))  # fake latents
        return softmax_actions, dict(mean=means, log_std=log_stds, latents=latents)
