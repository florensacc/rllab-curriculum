...
class GatherViewer(MjViewer):
    ...
    def render(self):
        ...
        apple_readings, bomb_readings = self.env.get_readings(vector=False)
...
class FollowEnv(ProxyEnv, Serializable):
    MODEL_CLASS = None
    ORI_IND = None
    ...
    def get_readings(self, vector=None):  # equivalent to get_current_maze_obs in maze_env.py
        if vector is None:
            vector = self.goal_vector_obs
    ...
    def get_current_obs(self):
        ...
        return np.concatenate([self_obs, *self.get_readings()])
