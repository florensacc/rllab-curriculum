...
for n in [2]:
    ...
    for resample in [True]:
        for reward_coef in [0, 0.01, 0.1, 1]:
            for latent_dim in [1, 2, 5]:
                for latent_name in ['bernoulli','normal']:
                    for n_samples in [0,1,2]:
                        ...
                        for s in [4,5,155]:
                            if resample:
                                exp_name = 'snn_{}_{}rewcoef_{}batch_{}latent_{}_{}hallu_{:04d}'.format(
                                    'Resamp', reward_coef, 500, latent_dim, latent_name, n_samples, s)
                            else:
                                exp_name = 'snn_{}_{}rewcoef_{}batch_{}latent_{}_{}hallu_{:04d}'.format(
                                    'NoResamp', reward_coef, 500, latent_dim, latent_name, n_samples, s)
                            run_experiment_lite(
