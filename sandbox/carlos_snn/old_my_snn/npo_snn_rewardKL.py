...
class NPO_snn_kl(BatchPolopt):
    ...
    def __init__(
        ...
        super(NPO_snn_kl, self).__init__(**kwargs)
    ...
    def init_opt(self):
        ...
        for dist_info_vars1 in list_dist_info_vars:
            list_kls_var1 = []
            for dist_info_vars2 in list_dist_info_vars:
            ...
            # list_kls.append(TT.mean(list_kls_var1))
    ...
    def log_diagnostics(self, paths):
        ...
        if self.policy.latent_dim:
            ...
            print(np.max(np.mean(all_kls, axis=0), axis=-1))  # this could also be a mean, no difference

            all_means = np.mean([path['agent_infos']['mean'] for path in paths], axis=(0, 1))
            print(all_means)
            # list_log_stds = np.mean([self._list_log_stds(path['observations']) for path in paths], axis=[0, 1])
            # for i, log_std in enumerate(list_log_stds):
            #     logger.record_tabular('AvgLogStd_lat{}'.format(i), log_std)
