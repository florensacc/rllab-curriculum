...
there is an optimal policy (stop moving), and we would like to see the improvement in value
estimation lead to faster convergence to that policy."""
...
def experiment_variant_generator():
    vg = VariantGenerator()
    vg.add("env_noise", [0, 0.05, 0.1, 0.5, 1])
    vg.add("env",
           lambda env_noise: [normalize(NoisyPointEnv(env_noise=env_noise,
                                                      end_early=False,
