...
info = """Using extreme linear basleine in a setting in which I've analytically derived the exact
form of the value estimate, and it is linear. Confirming that the baseline fits perfectly. Here the
environment is noiseless."""
...
def experiment_variant_generator():
    vg = VariantGenerator()
    vg.add("batch_size", [1000, 4000, 10000], hide=True)
    ...
    vg.add("env",
           lambda start_state, action_scale: [normalize(StaticEnv(action_scale=action_scale,
                                                                  start_state=start_state))],
           hide=True)
    vg.add("baseline",
           lambda env, lookahead: [ExtremeLinearBaseline(env.spec, lookahead)],
           hide=True
           )
    return vg
