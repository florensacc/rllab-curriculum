...
class ParallelGaussianMLPRegressor(GaussianMLPRegressor):
    """
    ...
    def __init__(
        ...
        super(ParallelGaussianMLPRegressor, self).__init__(**kwargs)
    ...
    def init_par_objs(self, n_parallel):
        ...
        if self._normalize_inputs:
            ...
            par_data.append(
                x_sum_2d=np.reshape(mp.RawArray(
                    'd', int(input_dim * n_parallel)),
                    (input_dim, n_parallel)),
                x_square_sum_2d=np.reshape(mp.RawArray(
                    'd', int(input_dim * n_parallel)),
                    (input_dim, n_parallel)),
            )
        ...
        if self._normalize_outputs:
            par_data.append(
                y_sum_2d=np.reshape(mp.RawArray(
                    'd', int(self.output_dim * n_parallel)),
                    (self.output_dim, n_parallel)),
                y_square_sum_2d=np.reshape(mp.RawArray(
                    'd', int(self.output_dim * n_parallel)),
                    (self.output_dim, n_parallel)),
            )
