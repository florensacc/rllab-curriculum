...
from sandbox.dave.rllab.core.alexnet import VanillaConvNet
...
class GaussianConvMLPRegressor(LasagnePowered, Serializable):
    """
    ...
    def __init__(
        ...
        self._optimizer = ConjugateGradientOptimizer(**optimizer_args)
        ...
        self._layers_old = conv_network.layers
        if adaptive_std:
        ...
        for i, (name, layer) in enumerate(conv_network.layers.items()):
            if name in ['conv1', 'conv2_part1', 'conv2_part2', 'conv3', 'conv4_part2', 'conv4_part2', 'conv5_part1',
                        'conv5_part2']:
        ...
        normalized_means_var = L.get_output(
            l_mean, {l_input: normalized_xs_var})
        normalized_log_stds_var = L.get_output(
            l_log_std, {l_input: normalized_xs_var})
    ...
    def log_likelihood_sym(self, x_var, y_var):
        ...
        normalized_means_var, normalized_log_stds_var = \
            L.get_output([self._l_mean, self._l_log_std], {
                self._input_layer: normalized_xs_var})
