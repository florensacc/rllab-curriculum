...
import os
import time
from rllab.mujoco_py import mjcore, mjconstants, glfw
...
class MjViewer(object):
    ...
    def render(self):
        ...
        if self.is_bot:
            depth_rgb = self.get_depth_map()
            gl.glDrawPixels(rect.height, rect.width, gl.GL_RGB, gl.GL_UNSIGNED_BYTE, depth_rgb)
            # mjlib.mjr_showBuffer(depth_rgb.ctypes.data_as(ctypes.POINTER(ctypes.c_ubyte)), rect.width, rect.height, 0, 0, byref(self.con))
