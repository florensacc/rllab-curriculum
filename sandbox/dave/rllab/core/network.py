...
class MLPtanh(LasagnePowered, Serializable):
    def __init__(self, output_dim, hidden_sizes, hidden_nonlinearity,
                 output_nonlinearity, hidden_W_init=LI.GlorotUniform(), hidden_b_init=LI.Constant(0.),
                 output_W_init=LI.GlorotUniform(), output_b_init=LI.Constant(0.),
                 name=None, input_var=None, input_layer=None, input_shape=None, batch_norm=False, beta=0.1):
        ...
        l_joint_angles = CropLayer(l_in, start_index=None, end_index=7)
        l_out = L.ElemwiseSumLayer([MultiplyScalarLayer(l_pre_out, beta),
                                            l_joint_angles])
        self._layers.append(l_pre_out)
        self._layers.append(l_out)
        self._l_in = l_in
        self._l_out = l_out
        # self._input_var = l_in.input_var
