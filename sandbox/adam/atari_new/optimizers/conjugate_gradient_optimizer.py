...
class ConjugateGradientOptimizer(Serializable):
    ...
    def optimize(self, inputs, extra_inputs=None, subsample_grouped_inputs=None):
        ...
        # Logging moved from NPO into here:
        if self._name:
            pre = self._name + "_"
        else:
            pre = ""
        logger.record_tabular(pre + 'LossBefore', loss_before)
        logger.record_tabular(pre + 'LossAfter', loss)
        logger.record_tabular(pre + 'MeanKLBefore', mean_kl_before)  # zero!
        logger.record_tabular(pre + 'MeanKL', constraint_val)
        logger.record_tabular(pre + 'dLoss', loss_before - loss)
