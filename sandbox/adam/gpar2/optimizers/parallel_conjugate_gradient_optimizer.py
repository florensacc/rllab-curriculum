...
from sandbox.adam.gpar2.optimizers import krylov
...
class ParallelConjugateGradientOptimizer(Serializable):
    ...
    def initialize_par_objs(self, n_parallel, grad_size):
        """
        Before forking subprocesses.
        Since all of the shared variables are used by all workers, just pass to
        workers by inheritance.
        """
        n = n_parallel
        self.n_parallel = n
        n_elm_per_worker = -(-grad_size // n)  # (ceiling div)
        ...
        m_arr = mp.RawArray
        shareds = struct(
            flat_g=n_arr(m_arr(tc, grad_size)),
            grads_2d=n_arr(m_arr(tc, n * grad_size)).reshape(n, grad_size),
            Hx=n_arr(m_arr(tc, grad_size)),
            loss=n_arr(m_arr(tc, n)),
            constraint_val=n_arr(m_arr(tc, n)),
            descent=n_arr(m_arr(tc, grad_size)),
            prev_param=n_arr(m_arr(tc, grad_size)),
            cur_param=n_arr(m_arr(tc, grad_size)),
            n_samples=m_arr('i', n),
        )
        barriers = struct(
    ...
    def initialize_rank(self, rank):
        ...
        if rank == (self.n_parallel - 1):  # (this one is the "master")
            cur_param[:] = self._target.get_param_values(trainable=True)
