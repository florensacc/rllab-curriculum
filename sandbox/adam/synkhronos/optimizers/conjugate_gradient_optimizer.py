...
from sandbox.adam.synkhronos.misc.ext import compile_synk_function
...
class PerlmutterHvp(Serializable):
    ...
    def update_opt(self, f, target, inputs, reg_coeff):
        ...
        # )
        self.opt_fun = dict(
            f_Hx_plain=compile_synk_function(
                inputs=inputs + xs,
                outputs=Hx_plain(),
                log_name="f_Hx_plain",
            )
    ...
    def build_eval(self, synk_inputs):
        if not hasattr(self, "synk_xs"):
            self.synk_xs = tuple([synk.data(x, scatter=False, force_cast=True) for x in self.xs])

        def eval(x):
            xs = tuple(self.target.flat_to_params(x, trainable=True))
            [synk_x.set_value(x, force_cast=True) for synk_x, x in zip(self.synk_xs, xs)]
            ret = self.opt_fun["f_Hx_plain"](
                *synk_inputs, *self.synk_xs, num_slices=self._num_slices)
...
class ConjugateGradientOptimizer(Serializable):
    ...
    def update_opt(self, loss, target, leq_constraint, inputs, extra_inputs=None, constraint_name="constraint", *args,
        ...
        self._opt_fun = dict(
            # f_loss=compile_synk_function(
            #     inputs=inputs + extra_inputs,
            #     outputs=loss,
            #     log_name="f_loss",
            # ),
            f_grad=compile_synk_function(
                inputs=inputs + extra_inputs,
                outputs=flat_grad,
                log_name="f_grad",
            ),
            # f_constraint=compile_synk_function(
            #     inputs=inputs + extra_inputs,
            #     outputs=constraint_term,
            #     log_name="f_constraint",
            # ),
            f_loss_constraint=compile_synk_function(
                inputs=inputs + extra_inputs,
                outputs=[loss, constraint_term],
                log_name="f_loss_constraint",
            ),
    ...
    def optimize(self, inputs, extra_inputs=None, subsample_grouped_inputs=None):
        ...
        if not hasattr(self, "synk_inputs"):
            self.synk_inputs = self._opt_fun["f_grad"].build_inputs(*inputs, force_cast=True)
            self.synk_subsample_inputs = self._opt_fun["f_grad"].build_inputs(*subsample_inputs, force_cast=True)
        else:
            for synk_input, inpt in zip(self.synk_inputs, inputs):
                synk_input.set_value(inpt, force_cast=True)
            for synk_sub_input, sub_inpt in zip(self.synk_subsample_inputs, subsample_inputs):
                synk_sub_input.set_value(sub_inpt, force_cast=True)

        logger.log("computing loss before")
        loss_before, mean_kl_before = self._opt_fun["f_loss_constraint"](
            *self.synk_inputs, num_slices=self._num_slices)
        # loss_before = sliced_fun(self._opt_fun["f_loss"], self._num_slices)(
        #     inputs, extra_inputs)
        logger.log("performing update")
        logger.log("computing descent direction")

        flat_g = self._opt_fun["f_grad"](
            *self.synk_inputs, num_slices=self._num_slices)
        # flat_g = sliced_fun(self._opt_fun["f_grad"], self._num_slices)(
        #     inputs, extra_inputs)

        Hx = self._hvp_approach.build_eval(self.synk_subsample_inputs)
        ...
        param_vars = self._target.get_params(trainable=True)
        ...
        for n_iter, ratio in enumerate(self._backtrack_ratio ** np.arange(self._max_backtracks)):
            ...
            synk.broadcast(param_vars)
            loss, constraint_val = self._opt_fun["f_loss_constraint"](
                *self.synk_inputs, num_slices=self._num_slices)
