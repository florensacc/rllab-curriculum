...
class PairwiseGpuSampler_2(BaseSampler):
    ...
    def init_par_objs(self, n_par, obs_dim, act_dim, state_info_keys):
        ...
        shareds = struct(
            ...
            act=[np.ctypeslib.as_array(
                ...
                for _ in range(2)],
            rew=[np.ctypeslib.as_array(mp.RawArray('f', n_par))
                for _ in range(2)],
            done=[mp.RawArray(c_bool, n_par) for _ in range(2)],
            continue_sampling=mp.RawValue(c_bool, True),
        )
        for k in state_info_keys:
            shareds[k] = [np.ctypeslib.as_array(mp.RawArray('f', n_par))
    ...
    def obtain_samples_master(self, itr):
        ...
        while continue_sampling:
            ...
            for rank in range_par:
                sims_act[j][rank].append(all_act[j][rank, :])  # use new 'all_act' each iteration instead of shared_act, to make sure it's not just a view to shared variable, which will change values
                sims_rew[j][rank].append(all_rew[j][rank])
                sims_obs[j][rank].append(all_obs[j][rank, :])
                sims_agent_info[j][rank].append({k: v[rank] for k, v in all_agent_info[j].items()})
                if all_done[j][rank]:
                    paths.append(dict(
                        observations=tensor_utils.stack_tensor_list(sims_obs[j][rank]),
                        actions=tensor_utils.stack_tensor_list(sims_act[j][rank]),
                        rewards=tensor_utils.stack_tensor_list(sims_rew[j][rank]),
                        agent_infos=tensor_utils.stack_tensor_dict_list(sims_agent_info[j][rank]),
                        env_infos={},
                    ))
                    cum_length_complete_paths += len(sims_rew[j][rank])
                    continue_sampling = (cum_length_complete_paths < self.batch_size)
                    sims_act[j][rank] = []
                    sims_obs[j][rank] = []
                    sims_rew[j][rank] = []
                    sims_agent_info[j][rank] = []
            if any(all_done[j]):
    ...
    def obtain_samples_simulator(self, rank, itr):
        ...
        while par.shareds.continue_sampling.value:
            ...
            o, r, d, env_info = self.algo.env[j].step(par.shareds.act[j][rank, :])
            # gt.stamp('step')
            # TODO: Later, might want to have the simulator skip an iteration to reset.
            if d:
                o = self.algo.env[j].reset()
                # gt.stamp('reset')

            # Share all the current data.
            par.shareds.obs[j][rank, :] = o
            par.shareds.rew[j][rank] = r
            par.shareds.done[j][rank] = d
