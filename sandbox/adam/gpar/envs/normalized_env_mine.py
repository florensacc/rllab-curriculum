...
class NormalizedEnv(ProxyEnv, Serializable):
    def __init__(
        ...
        if isinstance(self._wrapped_env.action_space, Box):
            ...
            self._action_space = spaces.Box(-1 * ub, ub)
            self._action_lb, self._action_ub = self._wrapped_env.action_space.bounds  # assume static
            self._action_range = self._action_ub - self._action_lb
            self._action_space_is_box = True
        else:
            self._action_space = self._wrapped_env.action_space
            self._action_space_is_box = False
    ...
    @overrides
    def step(self, action):
        if self._action_space_is_box:
            # rescale the action
            scaled_action = self._action_lb + (action + 1.) * 0.5 * self._action_range
            scaled_action = np.clip(scaled_action, self._action_lb, self._action_ub)
        else:
            scaled_action = action
        wrapped_step = self._wrapped_env.step(scaled_action)
        next_obs, reward, done, info = wrapped_step
