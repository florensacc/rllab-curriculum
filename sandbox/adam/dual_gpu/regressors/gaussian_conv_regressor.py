""" Lasagne imports must move out of file header and inside class """
...
class GaussianConvRegressor(LasagnePowered, Serializable):
    ...
    def fit(self, xs, ys):
        ...
        for batch in iterate_minibatches_generic(input_lst=[xs, ys], batchsize=self._batchsize, shuffle=self._shuffle):
            batch_count += 1
            xs, ys = batch
    ...
    def log_likelihood_sym(self, x_var, y_var):
        ...
        normalized_means_var, normalized_log_stds_var = \
            self.L.get_output([self._l_mean, self._l_log_std], {
