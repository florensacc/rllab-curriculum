...
class ALE(object):
    ...
    Code adopted from @muupan: https://github.com/muupan/async-rl
    """

    def __init__(self, game_name, seed=None, use_sdl=False, n_last_screens=4,
    ...
    def current_screen(self):
        ...
        rgb_img = np.maximum(to_rgb(self.ale), self.last_raw_screen)
    ...
    def receive_action(self, action):
        ...
        for i in range(4):
            ...
            if i == 3:
                self.last_raw_screen = to_rgb(self.ale)
...
class ImageAtariEnv(Env, Serializable):
    def __init__(self, game_name, n_last_screens=4,
                 frame_skip=4, treat_life_lost_as_terminal=True,
                 crop_or_scale='scale', max_start_nullops=30):
        Serializable.quick_init(self, locals())

        self.ale = ALE(game_name=game_name, n_last_screens=n_last_screens, frame_skip=frame_skip,
                       treat_life_lost_as_terminal=treat_life_lost_as_terminal, crop_or_scale=crop_or_scale,
    ...
    def step(self, action):
        reward = self.ale.receive_action(action)
        done = self.ale.is_terminal
        new_obs = self.get_current_obs()
        return Step(new_obs, reward, done)

    def get_current_obs(self):
        return np.asarray(self.ale.state) / 255.0

    @property
    def observation_space(self):
        return Box(low=0., high=1., shape=(4, 84, 84))

    @property
    def action_space(self):
        return Discrete(self.ale.number_of_actions)
