...
class SummaryNetwork(LayersPowered, Serializable):
    def __init__(self, env_spec, obs_embedding_network, action_embedding_network, state_dim=100):
        Serializable.quick_init(self, locals())
        ...
        l_summary = L.GRULayer(
            l_embedding,
            num_units=state_dim,
            hidden_nonlinearity=tf.nn.relu,#tanh,#relu,#tanh,#relu,
        ...
        self.embedding_dim = obs_embedding_network.embedding_dim + action_embedding_network.embedding_dim
    ...
    def get_output(self, obs_var=None, action_var=None, **kwargs):
        ...
        prev_action_var = tf.concat(
            1,
            [
                tf.zeros(tf.pack([tf.shape(action_var)[0], 1, tf.shape(action_var)[2]])),
                action_var[:, :-1, :]
            ]
        )
        ...
        return L.get_output(
            self.l_last_summary,
            {self.l_obs_input: obs_var, self.l_action_input: prev_action_var},
            **kwargs
        )
...
class ActionNetwork(LayersPowered, Serializable):
    def __init__(self, env_spec, summary_network):
        ...
        self.l_prev_action_input = summary_network.l_action_input
    ...
    def get_step_op(self, obs_var, **kwargs):
        flat_embedding_var = L.get_output(
            L.TemporalFlattenLayer(self.summary_network.l_embedding),
            {
                ...
                self.summary_network.l_action_input: tf.expand_dims(self.prev_action_var, 1),
            }, **kwargs
        )
