...
def gen_copy_sequence():
    inp = np.zeros((seq_length+1, n_bits+1))
    inp[:seq_length, :n_bits] = np.eye(n_bits)[np.random.randint(low=0, high=n_bits, size=seq_length)]
    inp[seq_length, n_bits] = 1
    out = np.copy(inp[:seq_length])
    return inp, out

print(gen_copy_sequence())
