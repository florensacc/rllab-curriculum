...
class JointActorCritic(LayersPowered):
    def __init__(self, env_spec, hidden_nonlinearity=tf.nn.relu, W=L.HeUniformInitializer(),
...
class SAAC(object):
    ...
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            vec_env = VecEnvExecutor(self.env, n=self.n_parallel, max_path_length=np.inf)
...
if __name__ == "__main__":
    ...
    algo = SAAC(env=env, test_env=test_env, policy=policy, vf=vf, n_parallel=4, t_max=20)
    logger.log("training")
    algo.train()
