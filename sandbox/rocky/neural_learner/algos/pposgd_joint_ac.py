...
class PPOSGD(BatchPolopt):
    ...
    def sliced_loss_kl(self, inputs, prev_states):
        loss, diags = self.optimizer.loss_diagnostics(inputs=inputs, prev_states=prev_states)
    ...
    def optimize_policy(self, itr, samples_data):
        ...
        if self.policy.recurrent:
            ...
            if getattr(self.policy, "record_prev_states", False):
                prev_states = agent_infos["prev_state"][:, 0, :]

        if self.log_loss_kl_before:
            logger.log("Computing loss / KL before training")
            surr_loss_before, kl_before = self.sliced_loss_kl(all_inputs, prev_states)
            logger.log("Computed")
        ...
        self.optimizer.optimize(all_inputs, callback=itr_callback, prev_states=prev_states)
        ...
        if self.log_loss_kl_after or self.use_line_search or self.adaptive_learning_rate:
            ...
            surr_loss_after, kl_after = self.sliced_loss_kl(all_inputs, prev_states)
            logger.log("Computed")
