...
class AsyncAlgo(Picklable):
    def __init__(
        ...
        self.unpicklable_list = ["env", "agent", "test_env",
                                 "test_agent"]  # only need to store cur_env and cur_agent, see train_one_process()
    ...
    def child_fresh_env_agent(self, test=False):
        ...
        if test:
            env = Serializable.clone(self.test_env)
        else:
            env = Serializable.clone(self.env)
    ...
    def train_one_process(self, process_id, global_vars, args):
        ...
        try:
            ...
            while global_t < args["total_steps"]:
                ...
                if global_t == test_t:
                    scores, paths = self.evaluate_performance(
                    ...
                    logger.record_tabular_misc_stat('Return', scores, placement="front")
    ...
    def evaluate_performance(self, n_runs, horizon):
        ...
        for i in range(n_runs):
            ...
            t = 0
            while not terminal:
                action, agent_info = agent.act(obs, reward, terminal, extra)
                next_obs, reward, terminal, extra = env.step(action)
                scores[i] += reward
                t += 1
                observations.append(np.copy(obs))
                actions.append(np.copy(action))
                rewards.append(reward)
                env_infos.append(extra)
