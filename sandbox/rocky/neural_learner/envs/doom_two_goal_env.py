...
def create_maps(scale=1, maze_sizes=None, *args, **kwargs):
    ...
    for choice in [0, 1]:
        player = Thing(x=int(96 * 1.5), y=int(96 * 2.5), type=1, id=1)
        # this is the goal the player should reach
        # some candidate things:
        # 5 - blue keycard (kind of too small)

        if choice == 0:
            bluecard = Thing(x=int(96 * 1.5), y=int(96 * 0.5), z=20, type=5, id=2, scale=scale)
        else:
            bluecard = Thing(x=int(96 * 1.5), y=int(96 * 4.5), z=20, type=5, id=2, scale=scale)
        ...
        sio = StringIO()
        textmap.write(sio)
        maps.append(sio.getvalue())

    return maps


def mkwad(version, scale=1):
    resource_name = "doom/two_goal_env/scale_{scale}_{version}.wad".format(scale=scale, version=version)

    def mk():
        maps = create_maps(scale=scale)
        script = SCRIPT
        ...
        level_names = ["MAP%02d" % idx for idx in range(len(maps))]

        for idx, level_name in enumerate(level_names):
            wad.lumps.append(Lump(name=level_name, content=b''))
            wad.lumps.append(Lump(name="TEXTMAP", content=maps[idx].encode()))
...
class DoomTwoGoalEnv(DoomEnv, Serializable):
    def __init__(self, scale=1, living_reward=-0.01, *args, **kwargs):
        Serializable.quick_init(self, locals())
        self.living_reward = living_reward
        self.scale = scale
        self.doom_scenario_path = resource_manager.get_file(*mkwad(scale=scale, version="v4"))
    ...
    def vec_env_executor(self, n_envs):
        return VecDoomTwoGoalEnv(env=self, n_envs=n_envs)


class VecDoomTwoGoalEnv(VecDoomEnv):
    def step(self, action_n, max_path_length):
        next_obs, rewards, dones, infos = VecDoomEnv.step(self, action_n, max_path_length)
        normal_reward = self.env.living_reward * self.env.frame_skip
        infos['success'] = np.cast['int'](rewards > normal_reward + 1e-2)
