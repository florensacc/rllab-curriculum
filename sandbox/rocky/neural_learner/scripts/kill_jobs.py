...
exps = load_exps_data([os.path.join(config.PROJECT_PATH, "data/s3/doom-bandit-18")])#doom-maze-61")])
...
for job in jobs:
    ...
    if "doom-bandit-18" in job_name:
        exp = [exp for exp in exps if exp.params['exp_name'] == job_name][0]
        if exp.params['action_set'] in ['direction']:#50, 100, 200, 400]:
            job_list.append(job)
