...
for K, T in sorted(selected_settings, key=lambda x: x[1]):
    ...
    best_mean = max([mean for mean, _ in results[(K, T)]])
    best_std = [std for mean, std in results[(K, T)] if mean == best_mean][0]

    components = []

    for mean, std in results[(K, T)]:
        # perform a t-test
