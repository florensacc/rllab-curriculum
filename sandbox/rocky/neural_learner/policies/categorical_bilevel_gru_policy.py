...
class CategoricalBilevelGRUPolicy(StochasticPolicy, LayersPowered, Serializable):
    def __init__(
            self,
            name,
            env_spec,
        ...
        """
        Maintain two GRUs, one across episodes, and one within episodes
        Question: do we use the same GRU when summarizing a single episode?
        """
        """
        ...
        with tf.variable_scope(name):
            ...
            super(CategoricalBilevelGRUPolicy, self).__init__(env_spec)
            ...
            """
            episode_network = GRUNetwork(....)
            trial_network = GRUNetwork(...)

            def get_action(observation):
                # observation is a tuple: (obs, last_action, last_reward, last_terminal)
                obs, last_action, last_reward, last_terminal = observation
                if last_terminal:
                    # update trial-level hidden state
                    # reset the episode-level hidden state
                # update episode-level hidden state and compute action
            """
