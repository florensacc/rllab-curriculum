...
"""
Try larger frame skips on hex task
"""
...
class VG(VariantGenerator):
    ...
    def batch_size(self):
        if MODE == "local":
            return [1000]
        return [10000]#, 50000]#250000]  # , 50000]
    ...
    @variant
    def frame_skip(self, stochastic_frame_skips):
        if stochastic_frame_skips is None:
            return [4, 8, 16, 32]
        return [None]

    @variant
    def stochastic_frame_skips(self):
        yield [4, 8, 16, 32]
        yield None
        ...
        env = TfEnv(
            MultiEnv(
                wrapped_env=DoomHexGoalFindingMazeEnv(
                    ...
                    stochastic_frame_skips=v["stochastic_frame_skips"],
                    margin=v["margin"],
                    version="v8"
                ),
