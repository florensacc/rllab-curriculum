...
class VG(VariantGenerator):
    @variant
    def seed(self):
        return [10 * x + 1 for x in range(10)]
    ...
    @variant
    def rand_angle_amount(self):
        return [0.1]#0.05, 0.1, 0.2, 0.3]
...
for idx, vv in enumerate(variants):
        ...
        env = TfEnv(
            MultiEnv(
                wrapped_env=DoomBanditEnv(
                    ...
                    rand_angle_amount=v["rand_angle_amount"],
                ),
    ...
    config.AWS_INSTANCE_TYPE = random.choice(["c4.8xlarge", "m4.10xlarge"])
    config.AWS_SPOT = True
    config.AWS_SPOT_PRICE = '1.0'
    config.AWS_REGION_NAME = random.choice(["ap-northeast-2", "us-east-2", "ap-south-1", "us-east-1", "us-west-1"])
