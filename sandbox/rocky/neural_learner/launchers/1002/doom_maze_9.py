...
class VG(VariantGenerator):
    ...
    @variant
    def clip_lr(self):
        return [1.0]#0.3, 1.0]  # 0.1, 0.3, 1.0]  # , 0.2]
...
for vv in variants:  # [:10]:
    ...
    def run_task(v):
        ...
        env = TfEnv(DoomGoalFindingMazeEnv(vectorized=v["env_vectorized"]))
        ...
        algo = PPOSGD(
            ...
            min_n_epochs=2,
            use_line_search=v["use_line_search"],
            optimizer=TBPTTOptimizer(
                batch_size=v["opt_batch_size"],
                n_steps=OPT_N_STEPS,
