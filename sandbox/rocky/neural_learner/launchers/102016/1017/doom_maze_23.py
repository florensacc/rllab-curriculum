...
class VG(VariantGenerator):
    ...
    hidden_dims = np.random.choice([32, 64, 128, 256, 512], size=N_RNGS)
    horizon_schedule_spans = np.random.choice([0, 100], size=N_RNGS)
    batch_sizes = np.random.choice([10000, 50000], size=N_RNGS)
...
for vv in variants:  # [:2]:
    ...
    def run_task(v):
        ...
        env = TfEnv(
            MultiEnv(
                wrapped_env=DoomGoalFindingMazeEnv(rescale_obs=v["rescale_obs"]),
