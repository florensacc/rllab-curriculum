...
"""
Test on atari to better calibrate the current policy / algorithm
"""
...
for vv in variants:
    ...
    def run_task(v):
        ...
        env = TfEnv(AtariEnv(game="breakout", terminate_on_life_lost=True))
        eval_env = TfEnv(AtariEnv(game="breakout", terminate_on_life_lost=False))
        ...
        if v["softmax_param"] == "default":
            action_param = SoftmaxDefault(dim=env.action_dim)
        elif v["softmax_param"] == "normalized":
            action_param = SoftmaxNormalized(dim=env.action_dim, bias=1.)
        elif v["softmax_param"] == "exact_entropy":
            action_param = SoftmaxExactEntropy(
                dim=env.action_dim,
                input_dependent=False,
                initial_entropy_percentage=0.9,
            )
        elif v["softmax_param"] == "exact_entropy_dependent":
            action_param = SoftmaxExactEntropy(dim=env.action_dim, input_dependent=True)
        else:
