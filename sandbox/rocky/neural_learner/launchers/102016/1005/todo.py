...
"""
Test multiple episode setting in doom
"""
...
for vv in variants:#[:2]:
    ...
    def run_task(v):
        ...
        env = TfEnv(MultiEnv(
            DoomGoalFindingMazeEnv(restart_game=False),
            episode_horizon=100,
            n_episodes=5,
            discount=0.99,
        ))

        def feature_network(name, output_dim):
        ...
        baseline = L2RNNBaseline(
            name="vf",
            env_spec=env.spec,
            feature_network=feature_network("vf_network", output_dim=1),
        ...
        policy = CategoricalRNNPolicy(
            ...
            feature_network=feature_network("embedding_network", output_dim=256),
            name="policy"
        )
