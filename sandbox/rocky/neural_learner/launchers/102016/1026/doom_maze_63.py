...
from sandbox.rocky.neural_learner.policies.episodic_rnn_policy import EpisodicRNNPolicy
from sandbox.rocky.neural_learner.policies.modular_policy import ModularPolicy
from sandbox.rocky.tf.core.network import ConvMergeNetwork, ConvNetwork
...
for idx, vv in enumerate(variants):
    ...
    def run_task(v):
        ...
        class NetworkBuilder(EpisodicNetworkBuilder):
            def new_obs_feature_layer(self, l_obs):
                l_flat_obs = L.TemporalFlattenLayer(l_obs)
                l_flat_obs = L.reshape(l_flat_obs, ([0],) + img_shape)
                l_flat_feature = ConvNetwork(
                    name="obs_feature",
                    input_shape=img_shape,
                    input_layer=l_flat_obs,
                    output_dim=v["hidden_dim"],
                ...
                return L.TemporalUnflattenLayer(l_flat_feature, l_obs)
                # return l_obs
        ...
        if v["cell_type"] == "episodic_gru":
            cell = EpisodicCell(
                GRUCell(
                    num_units=v["hidden_dim"],
                    weight_normalization=True,
                    activation=getattr(tf.nn, v["nonlinearity"])
                )
            )
        elif v["cell_type"] == "gru":
            cell = GRUCell(
                num_units=v["hidden_dim"],
                weight_normalization=True,
                activation=getattr(tf.nn, v["nonlinearity"])
        ...
        policy = EpisodicRNNPolicy(
            ...
            env_spec=env.spec,
            cell=cell,
            network_builder=NetworkBuilder(env.spec)
        )
