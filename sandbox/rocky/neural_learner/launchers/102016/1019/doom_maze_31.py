...
USE_GPU = True  # False#True#False#True
...
N_RNGS = 50#100#150#200  # 80#38#200


class VG(VariantGenerator):
    entropy_bonus_coeffs = np.random.choice([0], size=N_RNGS)
    discounts = np.random.choice([0.99], size=N_RNGS)
    gae_lambdas = np.random.choice([1.], size=N_RNGS)
    mean_kls = np.random.choice([0.01], size=N_RNGS)
    clip_lrs = np.random.choice([0.3], size=N_RNGS)
    opt_batch_sizes = np.random.choice([64], size=N_RNGS)
    min_epochss = np.random.choice([5], size=N_RNGS)
    hidden_dims = np.random.choice([32, 128], size=N_RNGS)
    horizon_schedule_spans = np.random.choice([0], size=N_RNGS)
    batch_sizes = np.random.choice([100000], size=N_RNGS)
    nonlinearities = np.random.choice(["relu"], size=N_RNGS)
    n_episodess = np.random.choice([10], size=N_RNGS)
    episode_horizons = np.random.choice([100], size=N_RNGS)
    n_repeatss = np.random.choice([1], size=N_RNGS)
    living_rewards = np.random.choice([-0.1, -0.01, -0.001], size=N_RNGS)
    ...
    @variant
    def n_repeats(self, rng_index):
        yield int(self.n_repeatss[rng_index])

    @variant
    def living_reward(self, rng_index):
        yield float(self.living_rewards[rng_index])
