...
for idx, vv in enumerate(variants):  # [:2]:
    ...
    def run_task(v):
        ...
        if v["softmax_param"] == "default":
            ...
            action_param = SoftmaxExactEntropy(
                ...
                initial_entropy_percentage=0.98,
            )
        else:
        ...
        n_envs = min(20, max(1, v["batch_size"] // v["max_path_length"]))
        ...
        algo = TRPO(
            ...
            optimizer=ConjugateGradientOptimizer(
                hvp_approach=FiniteDifferenceHvp(base_eps=v["base_eps"]),
                num_slices=20,
            ),
            step_size=0.01,
    ...
    if MODE == "local_docker":
        env = dict(CUDA_VISIBLE_DEVICES="7")
    else:
        env = dict()  # CUDA_VISIBLE_DEVICES="")

    run_experiment_lite(
        run_task,
        exp_prefix="doom-maze-38",
        mode=MODE,
        n_parallel=0,
        seed=vv["seed"],
        use_gpu=USE_GPU,
