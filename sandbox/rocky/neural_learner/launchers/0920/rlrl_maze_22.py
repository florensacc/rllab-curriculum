...
from sandbox.rocky.neural_learner.algos.pposgd import PPOSGD, SharedWeightsBaseline
...
for v in variants:
    ...
    baseline = SharedWeightsBaseline(env_spec=v["env"].spec, policy=policy)
    if v["algo_type"] == "trpo":
        algo = TRPO(
            env=v["env"],
            policy=policy,
