...
vg = VariantGenerator()
vg.add("seed", [x*100+11 for x in range(100)])#911])#11, 111, 211, 311, 411, 511, 611, 711, 811, 911])
vg.add("learning_rate", [1e-3])
vg.add("mi_coeff", [0.1]) #0, 0.001, 0.01, 0.1, 1., 10])
vg.add("ent_g_given_z_coeff", [0.0])#1.0]) #0., 0.001, 0.01, 0.1, 1., 10])
...
for v in variants:
    ...
    run_experiment_lite(
        algo.train(),
        exp_prefix="seq_grid_imitation_13_1",
        seed=v["seed"],
        variant=v,
        mode="lab_kube",
        snapshot_mode="last",
