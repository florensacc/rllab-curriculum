...
class ManualBottleneckLayer(L.Layer):
    def __init__(self, incoming, name, **kwargs):
        super(ManualBottleneckLayer, self).__init__(incoming, name, **kwargs)

    def get_output_for(self, input, **kwargs):
        # slices = tf.reshape(input, (-1, 3, 5))
        # # count the number of slices not equal to the placeholder
        # slices = [input[:, :5], input[:, 5:10], input[:, 10:15]]
        counts = tf.cast(tf.reduce_sum(tf.reduce_sum(tf.tile(np.eye(5, dtype=np.float32)[-1].reshape((1, 1, -1)),
                                                             tf.pack((tf.shape(input)[0], 3, 1))) * tf.reshape(input, (
            -1, 3, 5)), -1), -1), tf.int32)

        return tf.nn.embedding_lookup(np.eye(3, dtype=np.float32), 3 - counts)
...
class BranchingCategoricalMLPPolicy(StochasticPolicy, LayersPowered, Serializable):
    def __init__(
        ...
        with tf.variable_scope(name):
            ...
            # l_bottleneck = ManualBottleneckLayer(
            #     shared_network.input_layer,
            #     name="bottleneck"
            # )

            # l_bottleneck = MLP(
            #     name="bottleneck_network",
            #     input_shape=l_last.output_shape,
            #     input_layer=l_last,
            #     output_dim=bottleneck_dim,
            #     hidden_nonlinearity=tf.nn.tanh,
            #     output_nonlinearity=tf.nn.softmax,  # None,#tf.nn.tanh,
            #     hidden_sizes=(100, 100),
            #
            # ).output_layer
...
def generate_demonstration_trajectory(size, start_pos, end_pos, action_map):
    ...
    subgoals = []
    bottlenecks = []
    bottleneck_template = [0, 1, 2]
    if sx < ex:
        actions += action_map[DOWN] * (ex - sx)
        subgoals += [DOWN] * (ex - sx)
        bottlenecks += bottleneck_template * (ex - sx)
    elif sx > ex:
        actions += action_map[UP] * (sx - ex)
        subgoals += [UP] * (sx - ex)
        bottlenecks += bottleneck_template * (sx - ex)
    if sy < ey:
        actions += action_map[RIGHT] * (ey - sy)
        subgoals += [RIGHT] * (ey - sy)
        bottlenecks += bottleneck_template * (ey - sy)
    elif sy > ey:
        actions += action_map[LEFT] * (sy - ey)
        subgoals += [LEFT] * (sy - ey)
        bottlenecks += bottleneck_template * (sy - ey)
    ...
    return env, dict(
        ...
        subgoals=np.asarray(Discrete(4).flatten_n(subgoals)),
        bottlenecks=np.asarray(Discrete(3).flatten_n(bottlenecks)),
    )
...
class SeqGridExpert(object):
    ...
    def build_dataset(self, batch_size):
        if self.dataset is None:
            ...
            bottlenecks = tensor_utils.concat_tensor_list([p["bottlenecks"] for p in paths])
            N = observations.shape[0]
            seg_obs = observations.reshape((N / 3, 3, -1))
            seg_actions = actions.reshape((N / 3, 3, -1))
            seg_bottlenecks = bottlenecks.reshape((N / 3, 3, -1))
            subgoals = tensor_utils.concat_tensor_list([p["subgoals"] for p in paths])
            self.dataset = BatchDataset([seg_obs, seg_actions, subgoals, seg_bottlenecks], batch_size=batch_size)
            self.seg_obs = seg_obs
            self.seg_actions = seg_actions
            self.subgoals = subgoals
            self.seg_bottlenecks = seg_bottlenecks
        return self.dataset
    ...
    def log_exact_ent_g_given_z(self, algo):
        ...
        all_bottlenecks = np.asarray(all_bottlenecks)

        algo.g_given_z_regressor.fit(special.softmax(all_bottlenecks * 10000), np.asarray(all_subgoals))
        ent = np.mean(
...
class FixedClockImitation(RLAlgorithm):
    ...
    def surr_vlb_sym(self, obs_var, action_var, subgoals_var, bottlenecks_var):
        """
        ...
        # obs_var.set_shape((None, self.subgoal_interval, None))

        flat_obs_var = tf.reshape(obs_var, (-1, obs_dim))
        flat_action_var = tf.reshape(action_var, (-1, action_dim))

        subgoals_var = tf.cast(subgoals_var, tf.float32)
        bottlenecks_var = tf.cast(bottlenecks_var, tf.float32)

        high_obs_var = obs_var[:, 0, :]
        policy_subgoal_dist = self.high_policy.dist_info_sym(high_obs_var, dict())

        subgoal_logli = self.high_policy.distribution.log_likelihood_sym(subgoals_var, policy_subgoal_dist)

        low_obs = tf.concat(2, [obs_var, tf.tile(tf.expand_dims(subgoals_var, -2), (1, self.subgoal_interval, 1))])

        flat_low_obs = tf.reshape(low_obs, (-1, obs_dim + self.subgoal_dim))
        action_dist_info = self.low_policy.dist_info_sym(flat_low_obs)
        ...
        # lp_bottleneck = self.low_policy.bottleneck_sym(flat_obs_var)
        # true_bottleneck = tf.reshape(bottlenecks_var, (-1, 3))

        # bottlenecks_loss = tf.reduce_mean(true_bottleneck * -tf.log(lp_bottleneck + 1e-8))

        vlb = tf.reduce_mean(subgoal_logli) + tf.reduce_mean(sum_action_logli)  # - bottlenecks_loss
    ...
    def init_opt(self):
        ...
        subgoals_var = Discrete(4).new_tensor_variable(
            name="subgoal",
            extra_dims=1,
        )
        bottlenecks_var = Discrete(3).new_tensor_variable(
            name="bottleneck",
            extra_dims=2,
        )

        vlb, surr_vlb = self.surr_vlb_sym(obs_var, action_var, subgoals_var, bottlenecks_var)
        ...
        self.f_train = tensor_utils.compile_function(
            inputs=[obs_var, action_var, subgoals_var, bottlenecks_var],
