...
class FixedClockImitation(RLAlgorithm):
    ...
    def init_opt(self):
        ...
        ent_g_given_z, surr_ent_g_given_z = self.ent_g_given_z_sym(obs_var, action_var)
        ...
        joint_train_op = tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(
        ...
        #     - self.mi_coeff * mi_a_g_given_s, var_list=bottleneck_params
        # )
        ...
        train_op = tf.group(joint_train_op)  # , goal_recog_train_op)

        self.f_train = tensor_utils.compile_function(
            inputs=[obs_var, action_var],
            outputs=[train_op, vlb, mi_a_g_given_s, ent_g_given_z],
        )
            ...
            for epoch_id in xrange(self.n_epochs):
                ...
                for _ in xrange(self.n_sweep_per_epoch):
                    ...
                    for batch_obs, batch_actions in dataset.iterate():
                        ...
                        _, vlb_val, mi_val, ent_val = self.f_train(batch_obs, batch_actions)
                ...
                logger.record_tabular("H(g|z)", np.mean(ents))
