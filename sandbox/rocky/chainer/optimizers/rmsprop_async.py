...
class RMSpropAsync(optimizer.GradientMethod):
    ...
    def __init__(self, lr=0.01, alpha=0.99, eps=1e-8):
        ...
        self.alpha = alpha  # EMA weight on past
        self.eps = eps  # offset the denominator

    def init_state(self, param, state):
        xp = cuda.get_array_module(param.data)  # get numpy or cupy
        state['ms'] = xp.zeros_like(param.data)  # EMA of grad squares
