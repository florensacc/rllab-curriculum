...
def new_optimizer(chainer_optimizer_cls, chainer_optimizer_args, learning_rate):
    if chainer_optimizer_cls == chainer.optimizers.Adam:
        return chainer.optimizers.Adam(alpha=learning_rate, **chainer_optimizer_args)
    elif chainer_optimizer_cls == chainer.optimizers.RMSprop:
        return chainer.optimizers.RMSprop(lr=learning_rate, **chainer_optimizer_args)


def get_learning_rate(opt: chainer.Optimizer):
    if isinstance(opt, chainer.optimizers.Adam):
        return opt.alpha
    elif isinstance(opt, chainer.optimizers.RMSprop):
        return opt.lr
    else:
...
class TBPTTOptimizer(Serializable):
    ...
    def __init__(
        ...
        if chainer_optimizer is None:
            chainer_optimizer = chainer.optimizers.Adam(alpha=1e-3)
        self.chainer_optimizer = chainer_optimizer
        self.learning_rate = get_learning_rate(chainer_optimizer)
    ...
    def update_opt(self, f_loss_diags, target, **kwargs):
        """
        ...
        self.chainer_optimizer.setup(target)
        if self.gradient_clipping is not None:
            self.chainer_optimizer.add_hook(chainer.optimizer.GradientClipping(threshold=self.gradient_clipping))
