...
from sandbox.rocky.unn_hrl.sampler import parallel_sampler_vbnn as parallel_sampler
...
import time
from sandbox.rocky.unn_hrl.dynamics_models.nn_uncertainty import vbnn
# -------------------
...
#             assert self.kl_batch_size == 1
    ...
    def process_samples(self, itr, paths):
        ...
        if not self.policy.recurrent:
            ...
            undiscounted_total_returns = [sum(path["rewards"]) for path in paths]
        ...
        self.cum_tot_return += np.sum(undiscounted_total_returns)
        ...
        logger.record_tabular('AverageTotalReturn', np.mean(undiscounted_total_returns))
        logger.record_tabular('CumulatedTotalReturn', self.cum_tot_return)
