...
class StateGoalBonusEvaluator(object):
    """
    Compute the bonus to be I(g;s'|s)
    """

    def __init__(self, env_spec, subgoal_dim, regressor_cls=None, regressor_args=None):
        obs_dim = env_spec.observation_space.flat_dim
        action_dim = env_spec.action_space.flat_dim
        if regressor_cls is None:
            regressor_cls = BernoulliMLPRegressor
    ...
    def predict(self, path):
        ...
        return np.append(mi_est, 0)  # / (self.bonus_scale + 1e-8), 0)
