...
class BottleneckStateGoalSurpriseBonusEvaluator(object):
    ...
    def __init__(
        ...
        self.stk_given_st_regressor = regressor_cls(
            input_shape=(obs_dim,),
            output_dim=obs_dim,
            name="st1_given_st_regressor",
            optimizer=FirstOrderOptimizer(max_epochs=max_epochs, batch_size=128, verbose=True),
            **regressor_args
        )
        self.stk_given_st_gt_regressor = regressor_cls(
            input_shape=(obs_dim + subgoal_dim,),
            output_dim=obs_dim,
            name="st1_given_st_gt_regressor",
            optimizer=FirstOrderOptimizer(max_epochs=max_epochs, batch_size=128, verbose=True),
            **regressor_args
        )
        self.old_stk_given_st_gt_regressor = regressor_cls(
            input_shape=(obs_dim + subgoal_dim,),
            output_dim=obs_dim,
            name="old_st1_given_st_gt_regressor",
            optimizer=FirstOrderOptimizer(max_epochs=max_epochs, batch_size=128, verbose=True),
            **regressor_args
        )
        ...
        self.predict_interval_ahead = predict_interval_ahead

    def _extract_bottlenecks(self, obs):
        map_obs = obs[:, :self.map_obs_dim]
        N = map_obs.shape[0]
        history_obs = obs[:, self.map_obs_dim:self.map_obs_dim+self.history_obs_dim]
        history_obs = history_obs.reshape((-1, self.action_dim + 1))
        action_ids = special.from_onehot_n(history_obs)
        action_masks = np.cast['int'](action_ids != self.action_dim)
        counts = action_masks.reshape((N, self.subgoal_interval)).sum(axis=-1)
        flat_counts = np.eye(self.subgoal_interval)[counts]
        return np.concatenate([map_obs, flat_counts], axis=1)

    def predict(self, path):

        if self.predict_interval_ahead:
            k = self.subgoal_interval
        else:
            k = 1

        st = path["observations"][:-k:k]
        gt = path["agent_infos"]["subgoal"][:-k:k]
        st_gt = np.concatenate([st, gt], axis=1)
        stk = path["observations"][k::k]

        log_p_stk_given_st = self.stk_given_st_regressor.predict_log_likelihood(st, stk)
        log_p_stk_given_st_gt = self.stk_given_st_gt_regressor.predict_log_likelihood(st_gt, stk)

        log_p_old_stk_given_st_gt = self.old_stk_given_st_gt_regressor.predict_log_likelihood(st_gt, stk)

        mi_est = log_p_stk_given_st_gt - log_p_stk_given_st
        surprise_est = -log_p_old_stk_given_st_gt

        bonus = self.mi_coeff * mi_est + self.surprise_coeff * surprise_est

        return np.concatenate([bonus, [0] * (len(path["rewards"]) - len(bonus))])
    ...
    def fit_before_process_samples(self, paths):
        ...
        st = np.concatenate([p["observations"][:-k:k] for p in paths])
        gt = np.concatenate([p["agent_infos"]["subgoal"][:-k:k] for p in paths])
        stk = np.concatenate([p["observations"][k::k] for p in paths])
        st_gt = np.concatenate([st, gt], axis=1)

        self.old_stk_given_st_gt_regressor.set_param_values(
            self.stk_given_st_gt_regressor.get_param_values()
        )

        logger.log("Fitting p(st+k|st)...")
        self.stk_given_st_regressor.fit(st, stk)
        logger.log("Fitting p(st+k|st)...")
        self.stk_given_st_gt_regressor.fit(st_gt, stk)
    ...
    def log_diagnostics(self, paths):
        ...
        ent_stk_given_st = np.mean(-self.stk_given_st_regressor.predict_log_likelihood(st, stk))
        ent_stk_given_st_gt = np.mean(-self.stk_given_st_gt_regressor.predict_log_likelihood(st_gt, stk))

        cross_ent_old_stk_given_st_gt = np.mean(-self.old_stk_given_st_gt_regressor.predict_log_likelihood(st_gt, stk))

        mi_stk_gt_given_st = ent_stk_given_st - ent_stk_given_st_gt
        logger.record_tabular("H(st+k|st)", ent_stk_given_st)
        logger.record_tabular("H(st+k|st,gt)", ent_stk_given_st_gt)
        logger.record_tabular("I(st+k;gt|st)", mi_stk_gt_given_st)
        logger.record_tabular("KL(p(s)_new||p(s)_old)", cross_ent_old_stk_given_st_gt - ent_stk_given_st_gt)
