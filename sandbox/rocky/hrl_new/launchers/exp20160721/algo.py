...
class BottleneckStateGoalSurpriseBonusEvaluator(object):
    """
    Compute the bonus to be I(g;z'|z) - log(p_old(s'|g,s))
    """
    ...
    def __init__(
            ...
            regressor_args=None,
            mi_coeff=0.,
            bottleneck_mi_coeff=1.,
            surprise_coeff=1.,
            bottleneck_surprise_coeff=0.,
    ):
        obs_dim = env_spec.observation_space.flat_dim
        map_obs_dim = env_spec.observation_space.components[0].flat_dim
        history_obs_dim = obs_dim - map_obs_dim
        bottleneck_dim = map_obs_dim + subgoal_interval
        ...
        self.bottleneck_mi_coeff = bottleneck_mi_coeff
        self.surprise_coeff = surprise_coeff
        self.bottleneck_surprise_coeff = bottleneck_surprise_coeff
        self.bottleneck_given_prev_regressor = regressor_cls(
            input_shape=(bottleneck_dim,),
            output_dim=bottleneck_dim,
            name="bottleneck_given_prev_regressor",
        ...
        self.bottleneck_given_goal_prev_regressor = regressor_cls(
            input_shape=(bottleneck_dim + subgoal_dim,),
            output_dim=bottleneck_dim,
            name="bottleneck_given_goal_prev_regressor",
            optimizer=FirstOrderOptimizer(max_epochs=10, batch_size=128, verbose=True),
            **regressor_args
        )
        self.old_bottleneck_given_goal_prev_regressor = regressor_cls(
            input_shape=(bottleneck_dim + subgoal_dim,),
            output_dim=bottleneck_dim,
            name="old_bottleneck_given_goal_prev_regressor",
            optimizer=FirstOrderOptimizer(max_epochs=10, batch_size=128, verbose=True),
            **regressor_args
        )
        ...
        self.fit_delayed = None

    def _extract_bottlenecks(self, obs):
        map_obs = obs[:, :self.map_obs_dim]
        history_obs = obs[:, self.map_obs_dim:self.history_obs_dim]
        import ipdb;
        ipdb.set_trace()
        return None
    ...
    def predict(self, path):
        ...
        goal = path["agent_infos"]["subgoal"][:-1]
        obs_goal = np.concatenate([obs, goal], axis=1)
        next_obs = path["observations"][1:]
        bottlenecks = self._extract_bottlenecks(obs)
        next_bottlenecks = self._extract_bottlenecks(next_obs)
        bottleneck_goal = np.concatenate([bottlenecks, goal], axis=1)

        log_p_zt_given_zt1 = self.bottleneck_given_prev_regressor.predict_log_likelihood(bottlenecks, next_bottlenecks)
        log_p_st_given_st1 = self.state_given_prev_regressor.predict_log_likelihood(obs, next_obs)
        log_p_zt_given_gt_zt1 = self.bottleneck_given_goal_prev_regressor.predict_log_likelihood(bottleneck_goal,
                                                                                                 next_bottlenecks)
        ...
        log_p_old_zt_given_gt_zt1 = self.old_bottleneck_given_goal_prev_regressor.predict_log_likelihood(
            bottleneck_goal, next_bottlenecks)

        bottleneck_mi_est = log_p_zt_given_gt_zt1 - log_p_zt_given_zt1
        mi_est = log_p_st_given_gt_st1 - log_p_st_given_st1
        surprise_est = -log_p_old_st_given_gt_st1
        bottleneck_surprise_est = -log_p_old_zt_given_gt_zt1

        bonus = self.bottleneck_mi_coeff * bottleneck_mi_est + self.mi_coeff * mi_est + \
                self.surprise_coeff * surprise_est + self.bottleneck_surprise_coeff * bottleneck_surprise_est
    ...
    def fit_before_process_samples(self, paths):
        ...
        self.old_bottleneck_given_goal_prev_regressor.set_param_values(
            self.bottleneck_given_goal_prev_regressor.get_param_values()
        )

        self.bottleneck_given_prev_regressor.fit(bottlenecks, next_bottlenecks)
        self.bottleneck_given_goal_prev_regressor.fit(bottleneck_goal, next_bottlenecks)
    ...
    def log_diagnostics(self, paths):
        ...
        ent_state_given_goal_prev = np.mean(-self.state_given_goal_prev_regressor.predict_log_likelihood(obs_goal,
                                                                                                         next_obs))
        ent_bottleneck_given_prev = np.mean(-self.bottleneck_given_prev_regressor.predict_log_likelihood(bottlenecks,
                                                                                                         next_bottlenecks))
        ent_bottleneck_given_goal_prev = np.mean(-self.bottleneck_given_goal_prev_regressor.predict_log_likelihood(
            bottleneck_goal, next_bottlenecks))

        cross_ent_old_state_given_goal_prev = np.mean(-self.old_state_given_goal_prev_regressor.predict_log_likelihood(
            obs_goal, next_obs))
        cross_ent_old_bottleneck_given_goal_prev = np.mean(
            -self.old_bottleneck_given_goal_prev_regressor.predict_log_likelihood(
                bottleneck_goal, next_bottlenecks))

        mi_est = ent_state_given_prev - ent_state_given_goal_prev
        mi_bottleneck_est = ent_bottleneck_given_prev - ent_bottleneck_given_goal_prev
        logger.record_tabular("H(st+1|st)", ent_state_given_prev)
        logger.record_tabular("H(st+1|st,gt)", ent_state_given_goal_prev)
        logger.record_tabular("H(zt+1|zt)", ent_bottleneck_given_prev)
        logger.record_tabular("H(zt+1|zt,gt)", ent_bottleneck_given_goal_prev)
        logger.record_tabular("I(st+1;gt|st)", mi_est)
        logger.record_tabular("I(zt+1;gt|zt)", mi_bottleneck_est)
        logger.record_tabular("KL(p(s)_new||p(s)_old)", cross_ent_old_state_given_goal_prev - ent_state_given_goal_prev)
        logger.record_tabular("KL(p(z)_new||p(z)_old)", cross_ent_old_bottleneck_given_goal_prev -
                              ent_bottleneck_given_goal_prev)
