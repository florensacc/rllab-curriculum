...
class BottleneckStateGoalSurpriseBonusEvaluator(object):
    ...
    def predict(self, path):
        ...
        st = path["observations"][:-k]
        gt = path["agent_infos"]["subgoal"][:-k]
        st_gt = np.concatenate([st, gt], axis=1)
        stk = path["observations"][k:]
    ...
    def fit_before_process_samples(self, paths):
        ...
        st = np.concatenate([p["observations"][:-k] for p in paths])
        gt = np.concatenate([p["agent_infos"]["subgoal"][:-k] for p in paths])
        stk = np.concatenate([p["observations"][k:] for p in paths])
        st_gt = np.concatenate([st, gt], axis=1)
