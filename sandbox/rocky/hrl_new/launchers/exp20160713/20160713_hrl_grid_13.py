...
from sandbox.rocky.hrl_new.policies.fixed_clock_policy7 import FixedClockPolicy
...
"""
Use exact KL and limit low-level obs - a closer look
"""
...
class VG(VariantGenerator):
    ...
    @variant
    def bottleneck_dim(self, setup, subgoal_dim):
        if setup == "flat_policy":
            return [None]
        if subgoal_dim == 4:
            return [3]
    ...
    def env(self, env_size):
        ...
        env = TfEnv(wrapped_env)#, action_map, obs_include_history=True))
        yield env
