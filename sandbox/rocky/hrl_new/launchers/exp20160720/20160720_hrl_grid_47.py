...
from sandbox.rocky.hrl_new.launchers.exp20160720.algo2 import BonusTRPO, GoalBasedSurpriseBonusEvaluator, \
    FixedClockPolicy
...
class VG(VariantGenerator):
    @variant
    def seed(self):
        return [x * 100 + 11 for x in range(50)]
    ...
    @variant(hide=True)
    def policy(self, setup, env, hidden_sizes, subgoal_dim):
    ...
    @variant(hide=True)
    def algo(self, setup, env, policy, bonus_coeff, regressor_use_trust_region, subgoal_dim, use_mi, use_surprise):
        if setup == "flat_policy" or setup == "hrl_policy":
            ...
            yield BonusTRPO(
                ...
                discount=0.99,
                gae_lambda=1.0,
                bonus_evaluator=GoalBasedSurpriseBonusEvaluator(
                    env_spec=env.spec,
                    policy=policy,
                    subgoal_dim=subgoal_dim,
                    use_mi=use_mi,
