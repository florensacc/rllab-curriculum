...
class GoalBasedSurpriseBonusEvaluator(object):
    ...
    def predict(self, path):
        ...
        log_p_gt_given_st = np.log(np.sum(self.policy.f_subgoal_prob(obs) * goal, axis=-1) + 1e-8)
    ...
    def log_diagnostics(self, paths):
        ...
        ent_goal_given_state = np.mean(-np.log(np.sum(self.policy.f_subgoal_prob(obs) * goal, axis=-1) + 1e-8))
