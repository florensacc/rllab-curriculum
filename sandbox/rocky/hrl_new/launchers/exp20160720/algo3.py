...
class GoalBasedSurpriseBonusEvaluator(object):
    """
    ...
    def __init__(
        ...
        self.goal_state_regressor = regressor_cls(
            input_shape=(obs_dim,),
            output_dim=subgoal_dim,
            name="goal_state_regressor",
            **regressor_args
        )
        self.goal_regressor = regressor_cls(
            input_shape=(obs_dim + obs_dim,),
            output_dim=subgoal_dim,
            name="goal_regressor",
            **regressor_args
        )
        self.old_goal_regressor = regressor_cls(
            input_shape=(obs_dim + obs_dim,),
            output_dim=subgoal_dim,
            name="old_goal_regressor",
            **regressor_args
        )
    ...
    def predict(self, path):
        ...
        obs_next_obs = np.concatenate([path["observations"][:-1], path["observations"][1:]], axis=1)
        assert isinstance(self.policy, FixedClockPolicy)
        log_p_gt_given_st = self.goal_state_regressor.predict_log_likelihood(obs, goal)
        log_p_gt_given_st_next = self.goal_regressor.predict_log_likelihood(obs_next_obs, goal)
        log_p_old_gt_given_st_next = self.old_goal_regressor.predict_log_likelihood(obs_next_obs, goal)

        mi_est = log_p_gt_given_st_next - log_p_gt_given_st
        surprise_est = -log_p_old_gt_given_st_next

        bonus = np.zeros_like(mi_est)
        if self.use_mi:
            bonus += mi_est
    ...
    def fit_before_process_samples(self, paths):
        ...
        obs_next_obs = np.concatenate([obs, next_obs], axis=1)

        self.old_goal_regressor.set_param_values(
            self.goal_regressor.get_param_values()
        )
        self.goal_regressor.fit(obs_next_obs, goal)
        self.goal_state_regressor.fit(obs, goal)
    ...
    def log_diagnostics(self, paths):
        ...
        ent_goal_given_state = np.mean(-self.goal_state_regressor.predict_log_likelihood(obs, goal))
        ent_goal_given_state_next = np.mean(-self.goal_regressor.predict_log_likelihood(obs_next_obs, goal))
        cross_ent_old_goal_given_state_next = np.mean(-self.old_goal_regressor.predict_log_likelihood(obs_next_obs,
                                                                                                      goal))
        mi_est = ent_goal_given_state - ent_goal_given_state_next
        logger.record_tabular("H(gt|st)", ent_goal_given_state)
        logger.record_tabular("H(gt|st,st+1)", ent_goal_given_state_next)
        logger.record_tabular("I(st+1;gt|st)", mi_est)
        logger.record_tabular("KL(p_new||p_old)", cross_ent_old_goal_given_state_next - ent_goal_given_state_next)
