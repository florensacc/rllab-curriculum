...
class BiobjectiveConjugateGradientOptimizer(Serializable):
    ...
    def optimize(self, inputs, extra_inputs=None):
        ...
        if extra_inputs is None:
            extra_inputs = tuple()
        Hx = self.cg1._hvp_approach.build_eval(inputs + extra_inputs)

        adjusted_step_size = np.sqrt(
            2.0 * self.max_constraint_val * (1. / (initial_step.dot(Hx(initial_step)) + 1e-8))
        )
        if np.isnan(adjusted_step_size):
            adjusted_step_size = 1.

        initial_step = adjusted_step_size * initial_step
