...
class Trainer(Serializable):
    ...
    def init_opt(self):
        ...
        loss = tf.reduce_mean(-dist.log_likelihood_sym(action_var, minibatch_dist_info_vars))
        sqr_loss = tf.reduce_mean(tf.square(action_var - minibatch_dist_info_vars["mean"]))

        if len(state_output.get_shape().as_list()) == 3:
    ...
    def train(self, sess=None):
        ...
        def cb(itr, loss, best_loss, n_no_improvements, learning_rate, diagnostics):
            ...
            test_loss, test_diags = self.optimizer.loss_diagnostics([test_obs, test_actions])
            for k, v in test_diags.items():
                logger.record_tabular('Test' + k, v)
            ...
            if self.evaluate_performance:
                for det in [True, False]:
                    self.policy.configure(deterministic=det)
                    ...
                    self.policy.configure(deterministic=False)
                    if det:
                        prefix = "Det|"
                    else:
                        prefix = "Stoch|"
                    with logger.tabular_prefix(prefix):
                        logger.record_tabular(
                            'AveragePolicyReward',
                            np.mean([np.sum(p["rewards"]) for p in test_paths])
                        )
                        logger.record_tabular(
                            'SuccessRate',
                            np.mean([p["rewards"][-1] >= 4 for p in test_paths])
                        )
                        self.policy.log_diagnostics(test_paths)
