...
class ShapedCostTrainer(Serializable):
    def __init__(
            self,
            env,
            policy,
        ...
        self.use_shaped_cost = use_shaped_cost
    ...
    def init_opt(self, env, policy):
        ...
        analogy_diag_costs_var = tf.placeholder(dtype=tf.float32, shape=(None, None, None), name="analogy_diag_costs")
        ...
        def _loss_sym(action_var):
            if isinstance(env.action_space, Box):
                sqr_diff = tf.square(analogy_actions_var - action_var)
                shaped_sqr_diff = sqr_diff * analogy_diag_costs_var
                return tf.reduce_sum(
                    tf.reduce_sum(shaped_sqr_diff, -1) * analogy_valids_var
        ...
        def to_feed(batch_dict):
            ...
            if self.use_shaped_cost:
                analogy_diag_costs = tensor_utils.pad_tensor_n(
                    [p["diag_cost_expansions"] for p in analogy_paths], max_analogy_len
                )
            else:
                analogy_diag_costs = np.ones_like(analogy_actions)
            ...
            return {
                ...
                analogy_diag_costs_var: analogy_diag_costs,
    ...
    def collect_demos(self):
        ...
        exp_cost_expansions = data["exp_cost_expansions"]
        logger.log("Loaded")
        paths = []
        for xs, us, rewards, task_id, cost_expansions in zip(exp_x, exp_u, exp_rewards[:, :, 0], exp_task_ids,
                                                             exp_cost_expansions):
                ...
                if self.task_id_filter is None or task_id in self.task_id_filter:
                    ...
                    paths.append(
                        dict(
                            ...
                            diag_cost_expansions=np.clip(np.diagonal(cost_expansions, axis1=1, axis2=2), 1, 10),
                        )
                    )
    ...
    def train(self, sess=None):
        ...
        for epoch_idx in range(self.n_epochs):
            ...
            traj_lens = [len(p["rewards"]) for p in eval_paths]

            eval_actions = np.concatenate([p["actions"] for p in eval_paths], axis=0)
            action_norms = np.linalg.norm(eval_actions, axis=1)
            logger.record_tabular_misc_stat(key='Return', values=returns, placement='front')
            logger.record_tabular_misc_stat(key='TrajLen', values=traj_lens, placement='front')
            logger.record_tabular_misc_stat(key='ActionNorm', values=action_norms, placement='front')
