...
class GroundTruthAnalogyPolicy(ModulePowered, Serializable):
    ...
    def data_dependent_init(self, *args, **kwargs):
        pass
    ...
    def get_embedding(self, demo_task_ids):
        ...
        source_ids = ops.as_variable(np.asarray(source_ids), dtype='long')
        target_ids = ops.as_variable(np.asarray(target_ids), dtype='long')
    ...
    def inform_task(self, task_id, env, paths, obs):
        ...
        self.demo_embedding = ops.as_variable(demo_embedding)
    ...
    def get_actions(self, observations):
        ...
        flat_obs = ops.as_variable(flat_obs)
        ...
        agent_infos = {k: ops.to_numpy(v) for k, v in info_vars.items()}
