...
from sandbox.rocky.new_analogy.exp_utils import run_cirrascale, run_local_docker, run_local, run_ec2
...
def run_task(vv):
    ...
    with tf.Session() as sess:
        resource_name = "fetch_relative_dagger_pretrained_3_boxes_v1.pkl"
        ...
        env = fetch_utils.fetch_env(usage="rl", horizon=horizon, height=3)
...
for v in variants:
    # run_cirrascale(
    run_ec2(#local_docker(
        # run_local(
        run_task,
        exp_name="trpo-finetune-fetch-relative-3",
        variant=v,
        seed=v["seed"]
    )
    # exit()
