...
"""
Try state maching, use residual policies
"""
...
def run_task(v):
    ...
    from sandbox.rocky.new_analogy.tf.algos.trpo_with_eval_env import TRPOWithEvalEnv
    ...
    for k in [1, 5, 10, 20, 40, 80, 160, 320, 640, 1000]:
        ...
        with tf.Session() as sess:
            ...
            traj_reset_env = TrajResetEnv(env=gpr_env, paths=paths[:1], k=k)
            ...
            if policy_params is not None:
                ...
                if v["init_std"] is not None:
                    sess.run(tf.assign(policy._l_std_param.param, [np.log(v["init_std"])] * env.action_dim))
