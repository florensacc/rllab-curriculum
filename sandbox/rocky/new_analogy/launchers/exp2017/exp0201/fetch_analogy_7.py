...
OPTIMIZERS = OrderedDict([
    # ("adamax_1e-2", optimizers.Adamax(lr=1e-2)),
    # ("adamax_1e-3", optimizers.Adamax(lr=1e-3)),
    # ("adam_1e-2", optimizers.Adam(lr=1e-2)),
    # ("adam_1e-3", optimizers.Adam(lr=1e-3)),
    # ("rmsprop_1e-2", optimizers.RMSprop(lr=1e-2)),
    # ("rmsprop_1e-3", optimizers.RMSprop(lr=1e-3)),
...
def run_task(vv):
    ...
    from sandbox.rocky.new_analogy.tf.policies.final_state_analogy_policy import FinalStateAnalogyPolicy
    from sandbox.rocky.new_analogy.tf.policies.conv_analogy_policy import ConvAnalogyPolicy
    ...
    from sandbox.rocky.new_analogy.tf.algos.fetch_analogy_trainer import FetchAnalogyTrainer

    optimizer = optimizers.get(vv["optimizer_type"], kwargs=dict(lr=vv["learning_rate"]))
    FetchAnalogyTrainer(
