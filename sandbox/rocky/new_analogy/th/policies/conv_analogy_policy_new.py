...
class ConvAnalogyPolicy(ScopePowered, Serializable):
            ...
            for idx, rate in enumerate(self.rates):
                with ops.scope("res_block_{}".format(idx)):
                    x = (x.branch_out()
                         ...
                         .conv1d_wn(dim=dim // 2, size=1)
                         .act(F.relu)
                         .conv1d_wn(dim=dim // 2, size=self.filter_size, rate=rate, causal=True)
                         .act(F.relu)
                         .conv1d_wn(dim=dim, size=1)
                         .branch_in(mode='sum'))
            x = (x.act(F.relu)
                 .conv1d_wn(dim=dim, size=1)
                 .act(F.relu))
        with ops.scope("output"):
            x = x.conv1d(dim=self.action_flat_dim, size=1)
    ...
    def fast_get_actions(self, observations):
        ...
        with ops.scope(self.scope):
            ...
            if np.any(demo_to_compute):
                ...
                self.demo_embedding[
                    demo_to_compute] = tensor_utils.to_numpy(demo_embedding)
        ...
        agent_infos = {k: tensor_utils.to_numpy(
    ...
    def get_actions(self, observations):
        ...
        info_vars = self._forward(None, past_obs_arr, demo_embedding=tensor_utils.as_variable(
            demo_embedding))
