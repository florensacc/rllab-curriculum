...
# //////////// perform training //////////////
if not os.path.exists('/Users/rein/programming/openai/pixel-cnn'):
    os.makedirs('/Users/rein/programming/openai/pixel-cnn')
...
def conditioner_spec(x, init=False, ema=None):
    ...
    with scopes.arg_scope([nn.down_shifted_conv2d, nn.down_right_shifted_conv2d, nn.down_shifted_deconv2d,
        ...
        xs = nn.int_shape(x)
        x_pad = tf.concat(3,
                          [x, tf.ones(xs[:-1] + [1])])  # add channel of ones to distinguish image from padding later on
        u_list = [nn.down_shifted_conv2d(x_pad, num_filters=args.nr_filters,
        ...
        ul_list = [nn.down_shift(nn.down_shifted_conv2d(x_pad, num_filters=args.nr_filters, filter_size=[1, 3])) + \
                   nn.right_shift(nn.down_right_shifted_conv2d(x, num_filters=args.nr_filters,
...
# data
x_init = tf.placeholder(tf.float32, shape=(args.init_batch_size, im_size, im_size, 3))
...
x_sample = tf.placeholder(tf.float32, shape=(args.sample_batch_size, im_size, im_size, 3))
new_x_gen = nn.sample_from_discretized_mix_logistic(conditioner(x_sample, ema=ema), args.nr_logistic_mix)


def sample_from_model(sess):
    x_gen = np.zeros((args.sample_batch_size, im_size, im_size, 3), dtype=np.float32)
...
for i in range(args.nr_gpu):
    xs.append(tf.placeholder(tf.float32, shape=(args.batch_size, im_size, im_size, 3)))
...
bits_per_dim = loss[0] / (args.nr_gpu * np.log(2.) * 3 * im_size * im_size * args.batch_size)
bits_per_dim_test = loss_test[0] / (args.nr_gpu * np.log(2.) * 3 * im_size * im_size * args.batch_size)
