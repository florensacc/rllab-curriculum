...
def discretized_mix_logistic(x, l):
    ...
    log_probs = tf.select(x < -0.999, log_cdf_plus, tf.select(x > 0.999, log_one_minus_cdf_min,
                                                              tf.select(cdf_delta > 1e-3, tf.log(cdf_delta + 1e-7),
...
def sample_from_discretized_mix_logistic(l, nr_mix):
    ...
    sel = tf.one_hot(tf.argmax(
        logit_probs - tf.log(-tf.log(tf.random_uniform(logit_probs.get_shape(), minval=1e-5, maxval=1. - 1e-5))), 3),
                     depth=nr_mix, dtype=tf.float32)  # sample from softmax
...
def aux_gated_resnet(x, u, nonlinearity=concat_elu, conv=conv2d, **kwargs):
    num_filters = int(x.get_shape()[-1])
    c1 = conv(nonlinearity(x), num_filters, nonlinearity=None, **kwargs) + nin(nonlinearity(u), num_filters,
...
@scopes.add_arg_scope
def lstm(x, state=None, num_units=None, num_out=None, nonlinearity=tf.tanh, counters={}, init=False, ema=None,
         **kwargs):
    ...
    with tf.variable_scope(name):
        initial_lstm_c = get_var_maybe_avg('initial_lstm_c', ema, dtype=tf.float32, initializer=tf.zeros(num_units),
                                           trainable=True)
        initial_lstm_h = get_var_maybe_avg('initial_lstm_h', ema, dtype=tf.float32, initializer=tf.zeros(num_out),
                                           trainable=True)
        V = get_var_maybe_avg('V', ema, shape=[num_input + num_out, 4 * num_units], dtype=tf.float32,
    ...
    if process_single_step:
        ...
        W_x = W[:num_input, :]
        W_h = W[num_input:, :]
        x_times_W = tf.reshape(
            tf.matmul(tf.reshape(x, [xs[0] * xs[1], xs[2]]), W_x) + tf.reshape(b, [1, 4 * num_units]),
            [xs[0], xs[1], 4 * num_units])
