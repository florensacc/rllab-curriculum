...
if 'docker' in mode or 'ec2' in mode:
    base_model_dir = osp.join(config.DOCKER_CODE_DIR, base_model_dir)
else:
    base_model_dir = osp.join('/home/shhuang/src/rllab-private/', base_model_dir)
...
class VG(VariantGenerator):
    ...
    def adv_target_norm_fgsm_eps(self):
        ...
        #chosen_adv_target_norm_eps = chosen_adv_target_norm_eps[125:]  # Re-starting run after running out of hard-drive space
...
for v in variants:
    adv_model_dir, target_model_dir, norm, fgsm_eps = v['adv_target_norm_fgsm_eps']

    fgsm_eps = to_iterable(fgsm_eps) + [0]
    exp_prefix = "adv-rollouts/" + exp_index

    game = adv_model_dir.split('_')[-1]
    if game == 'invaders':
        game = 'space'
    if game == 'command':
        game = 'chopper'
