...
def new_loss(qfunc, boltz_policy, discount, obs, actions, rewards, terminate, prev_pdist, prev_qval, emp_qval, penalty):
    ...
    qval_loss = T.mean(T.square(qsa - y))
    joint_loss = policy_loss + qval_loss + penalty*mean_kl
...
class BPFQI(object):
    ...
    def train(self, mdp, qfunc):
        ...
        for itr in xrange(self.start_itr, self.n_itr):
            ...
            opt_info = self.optimize_policy(itr, boltz_policy, samples_data, opt_info)
    ...
    def init_opt(self, mdp, qfunc, boltz_policy):
        ...
        qval_loss_var = result_vars["qval_loss"]
        ...
        f_loss = theano.function(train_var_list, [policy_loss_var, qval_loss_var, mean_kl_var, joint_loss_var], allow_input_downcast=True, on_unused_input='ignore')
    ...
    def optimize_policy(self, itr, boltz_policy, samples_data, opt_info):
        ...
        def evaluate_loss(train_vals):
            def evaluate(params):
                ...
                policy_loss, qval_loss, mean_kl, joint_loss = f_loss(*train_vals)
        ...
        policy_loss_before, qval_loss_before, _, _ = f_loss(*(train_vals + [0]))
        loss_before = policy_loss_before + qval_loss_before
        ...
        logger.record_tabular('PerpBefore', np.mean(cat_perplexity(samples_data['prev_pdists'])))
        ...
        for penalty_itr in range(self.max_penalty_itr):
            ...
            try_policy_loss, try_qval_loss, try_mean_kl, try_joint_loss = f_loss(*(train_vals + [try_penalty]))

            logger.log('penalty %f => policy loss %f, qval loss %f, mean kl %f' % (try_penalty, try_policy_loss, try_qval_loss, try_mean_kl))
        ...
        policy_loss_after, qval_loss_after, _, _ = f_loss(*(train_vals + [penalty]))
        loss_after = policy_loss_after + qval_loss_after
