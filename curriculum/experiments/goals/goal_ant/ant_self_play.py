...
from curriculum.experiments.goals.goal_ant.ant_self_play_algo import run_task
...
if __name__ == '__main__':
    ...
    # setup ec2
    subnets = [
        'ap-southeast-2c', 'ap-southeast-2b'
    ]
    ec2_instance = args.type if args.type else 'm4.10xlarge' #'m4.10xlarge'
    # configure instan
    ...
    exp_prefix = 'maze-ant-goal-selfplay2'
    ...
    # replay buffer  # todo: is there replay in self-play? --> NO
    vg.add('replay_buffer', [False])
    vg.add('coll_eps', [0.3])
    vg.add('num_new_goals', [200])
    vg.add('num_old_goals', [100])
    vg.add('add_on_policy', [False])  # eliminate the other tricks of our method too
    vg.add('subsample_on_policy', lambda add_on_policy: [50] if add_on_policy else [0])
    # sampling params
    vg.add('horizon', [500])
