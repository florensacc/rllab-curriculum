...
from curriculum.envs.ant_goal.ant_target_env import AntEnv
...
def compute_final_states_from_paths(all_paths, as_goal=True, env=None):
    ...
    for paths in all_paths:
        for path in paths:
            if as_goal:
                state = path['env_infos']['obs2goal'][-1]
            else:
                logger.log("Not sure what to do here!!!")
                state = path['env_infos']['obs2start'][-1]
...
def run_task(v):
    ...
    inner_env = normalize(AntEnv(com_bound=v['goal_range']))  # todo: this does not take in goal_center!
