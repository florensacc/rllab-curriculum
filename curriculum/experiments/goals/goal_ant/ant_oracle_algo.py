...
from curriculum.envs.maze.maze_evaluate import test_and_plot_policy
...
def run_task(v):
    ...
    for outer_iter in range(1, v['outer_iters']):
        ...
        while goals.shape[0] < v['num_new_goals']:
            ...
            k += 1
            unif_goals = np.random.uniform(np.array(v['goal_center']) - np.array(v['goal_range']),
                                      np.array(v['goal_center']) + np.array(v['goal_range']), size=(unif_samples, v['goal_size']))
        ...
        logger.record_tabular('LabelingRollouts', k * v['n_traj'] * unif_samples)
