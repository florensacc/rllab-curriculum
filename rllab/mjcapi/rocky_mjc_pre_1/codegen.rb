...
def struct_regex(name, actual_name)
  /struct #{name}(\s+[^\n]*)?\n\{(.*?)\}\s+#{actual_name};/ms
end
...
def parse_struct(source, name, actual_name)
  source =~ struct_regex(name, actual_name)
  content = $2
  subs = []
...
structs = %w[_mjData _mjModel].map do |x|
  parse_struct(source, x, x[1..-1])
end
