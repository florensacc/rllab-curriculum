...
class RNNBaseline(Baseline, LasagnePowered, Serializable):
    ...
    def __init__(self, mdp):
        ...
        f_loss = compile_function(
            inputs=[l_in.input_var, returns_var, valids_var],
            outputs=loss,
        )
    ...
    def fit(self, paths):
        ...
        def evaluate(params):
            ...
            val = self._f_loss(obs, returns, valids)
            return val.astype(np.float64)

        def evaluate_grad(params):
            self.set_param_values(params)
            grad = self._f_grad(obs, returns, valids)
            flattened_grad = flatten_tensors(map(np.asarray, grad))
            return flattened_grad.astype(np.float64)

        cur_params = self.get_param_values()
        scipy.optimize.fmin_l_bfgs_b(
            func=evaluate, x0=cur_params, fprime=evaluate_grad, maxiter=100
        )
        self._trained = True
    ...
    def predict(self, path):
        if self._trained:
            obs = np.array([np.path["observations"]])
            return self._f_predict(obs)
        else:
