...
from rllab.mjcapi.rocky_mjc_1_22 import MjViewer, MjModel, mjcore, mjlib, \
    mjextra, glfw
from rllab.mdp.mujoco_1_22.gather.embedded_viewer import EmbeddedViewer
...
class GatherMDP(ControlMDP, Serializable):
    ...
    def step(self, state, action):
        next_state, _, _, done = \
            self.inner_mdp.step(state, action)
        if done:
            return next_state, self.get_current_obs(), -10, done
        ...
        return next_state, self.get_current_obs(), reward, done
